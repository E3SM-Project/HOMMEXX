...
struct CaarFunctorImpl {
  ...
  KOKKOS_INLINE_FUNCTION void compute_energy_grad(KernelVariables &kv, const Element& elem) const {
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&] (const int& ilev) {
        ...
        elem.buffers.energy_grad(0, igp, jgp, ilev) =
            PhysicalConstants::Rgas *
            (elem.buffers.temperature_virt(igp, jgp, ilev) /
             elem.buffers.pressure(igp, jgp, ilev)) *
            elem.buffers.pressure_grad(0, igp, jgp, ilev);

        elem.buffers.energy_grad(1, igp, jgp, ilev) =
            PhysicalConstants::Rgas *
            (elem.buffers.temperature_virt(igp, jgp, ilev) /
             elem.buffers.pressure(igp, jgp, ilev)) *
            elem.buffers.pressure_grad(1, igp, jgp, ilev);
        ...
        Scalar k_energy =
            0.5 * (elem.m_v(m_data.n0, 0, igp, jgp, ilev) *
                   ...
                   elem.m_v(m_data.n0, 1, igp, jgp, ilev) *
                       elem.m_v(m_data.n0, 1, igp, jgp, ilev));
        elem.buffers.ephi(igp, jgp, ilev) =
            k_energy + elem.m_phi(igp, jgp, ilev);
      });
    });
    kv.team_barrier();

    m_sphere_ops.gradient_sphere_update(kv,elem.buffers.ephi,elem.buffers.energy_grad);
  } // TESTED 1
  ...
  KOKKOS_INLINE_FUNCTION void compute_phase_3(KernelVariables &kv, const Element& elem) const {
    if (m_data.rsplit == 0) {
      // vertical Eulerian
      assign_zero_to_sdot_sum(kv,elem);
      compute_eta_dot_dpdn_vertadv_euler(kv,elem);
  ...
  void compute_velocity_np1(KernelVariables &kv, const Element& elem) const {
    ...
    m_sphere_ops.vorticity_sphere(kv, Homme::subview(elem.m_v, m_data.n0),elem.buffers.vorticity);
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&] (const int& ilev) {
        ...
        elem.buffers.vorticity(igp, jgp, ilev) +=
            elem.m_fcor(igp, jgp);

        elem.buffers.energy_grad(0, igp, jgp, ilev) *= -1;

        elem.buffers.energy_grad(0, igp, jgp, ilev) +=
            (rsplit_gt0 ? 0 : - elem.buffers.v_vadv_buf(0, igp, jgp, ilev)) +
            elem.m_v(m_data.n0, 1, igp, jgp, ilev) *
            elem.buffers.vorticity(igp, jgp, ilev);

        elem.buffers.energy_grad(1, igp, jgp, ilev) *= -1;

        elem.buffers.energy_grad(1, igp, jgp, ilev) +=
            (rsplit_gt0 ? 0 : - elem.buffers.v_vadv_buf(1, igp, jgp, ilev)) -
            elem.m_v(m_data.n0, 0, igp, jgp, ilev) *
            elem.buffers.vorticity(igp, jgp, ilev);

        elem.buffers.energy_grad(0, igp, jgp, ilev) *= m_data.dt;
        elem.buffers.energy_grad(0, igp, jgp, ilev) +=
            elem.m_v(m_data.nm1, 0, igp, jgp, ilev);
        elem.buffers.energy_grad(1, igp, jgp, ilev) *= m_data.dt;
        elem.buffers.energy_grad(1, igp, jgp, ilev) +=
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      for(int k = 0; k < NUM_LEV; ++k){
        elem.m_eta_dot_dpdn(igp, jgp, k) +=
           m_data.eta_ave_w * elem.buffers.eta_dot_dpdn_buf(igp, jgp, k);
      }//k loop
    });
    kv.team_barrier();
  ...
  KOKKOS_INLINE_FUNCTION
  void assign_zero_to_sdot_sum(KernelVariables &kv, const Element& elem) const {
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
                         KOKKOS_LAMBDA(const int idx) {
      const int igp = idx / NP;
      const int jgp = idx % NP;
      elem.buffers.sdot_sum(igp, jgp) = 0;
    });
    kv.team_barrier();
  } // TRIVIAL

  KOKKOS_INLINE_FUNCTION
  void compute_eta_dot_dpdn_vertadv_euler(KernelVariables &kv, const Element& elem) const {
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      for(int k = 0; k < NUM_PHYSICAL_LEV-1; ++k){
        ...
        elem.buffers.sdot_sum(igp, jgp) +=
           elem.buffers.div_vdp(igp, jgp, ilev)[ivec];
        elem.buffers.eta_dot_dpdn_buf(igp, jgp, ilevp1)[ivecp1] =
           elem.buffers.sdot_sum(igp, jgp);
      }//k loop
      ...
      for(int k = 1; k < NUM_PHYSICAL_LEV; ++k){
        const int ilev = k / VECTOR_SIZE;
        const int ivec = k % VECTOR_SIZE;
        elem.buffers.eta_dot_dpdn_buf(igp, jgp, ilev)[ivec] =
           m_hvcoord.hybrid_bi(k)*elem.buffers.sdot_sum(igp, jgp) -
           elem.buffers.eta_dot_dpdn_buf(igp, jgp, ilev)[ivec];
      }//k loop
      elem.buffers.eta_dot_dpdn_buf(igp, jgp, 0)[0] = 0.0;
    });//NP*NP loop
    kv.team_barrier();
...
//should be renamed
  ...
  void compute_div_vdp(KernelVariables &kv, const Element& elem) const {
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      const int jgp = idx % NP;
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&] (const int& ilev) {
        elem.buffers.vdp(0, igp, jgp, ilev) =
            elem.m_v(m_data.n0, 0, igp, jgp, ilev) *
            elem.m_dp3d(m_data.n0, igp, jgp, ilev);

        elem.buffers.vdp(1, igp, jgp, ilev) =
            elem.m_v(m_data.n0, 1, igp, jgp, ilev) *
            elem.m_dp3d(m_data.n0, igp, jgp, ilev);

        elem.m_derived_vn0(0, igp, jgp, ilev) +=
            m_data.eta_ave_w * elem.buffers.vdp(0, igp, jgp, ilev);

        elem.m_derived_vn0(1, igp, jgp, ilev) +=
            m_data.eta_ave_w * elem.buffers.vdp(1, igp, jgp, ilev);
      });
    });
    kv.team_barrier();

    m_sphere_ops.divergence_sphere(kv,elem.buffers.vdp,elem.buffers.div_vdp);
  } // TESTED 8
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&] (const int& ilev) {
        elem.m_omega_p(igp, jgp, ilev) +=
            m_data.eta_ave_w * elem.buffers.omega_p(igp, jgp, ilev);
      });
    });
    kv.team_barrier();
  } // TESTED 10
  ...
  void compute_temperature_np1(KernelVariables &kv, const Element& elem) const {
    ...
    m_sphere_ops.gradient_sphere(kv,Homme::subview(elem.m_t, m_data.n0),elem.buffers.temperature_grad);
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&] (const int& ilev) {
        const Scalar vgrad_t =
            elem.m_v(m_data.n0, 0, igp, jgp, ilev) *
                elem.buffers.temperature_grad(0, igp, jgp, ilev) +
            elem.m_v(m_data.n0, 1, igp, jgp, ilev) *
                elem.buffers.temperature_grad(1, igp, jgp, ilev);

        const Scalar ttens =
              (rsplit_gt0 ? 0 : -elem.buffers.t_vadv_buf(igp, jgp,
                                                             ilev)) -
            vgrad_t +
            PhysicalConstants::kappa *
                elem.buffers.temperature_virt(igp, jgp, ilev) *
                elem.buffers.omega_p(igp, jgp, ilev);
  ...
  void compute_dp3d_np1(KernelVariables &kv, const Element& elem) const {
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
                           [&](const int &ilev) {
        Scalar tmp = elem.buffers.eta_dot_dpdn_buf(igp, jgp, ilev);
        tmp.shift_left(1);
        tmp[VECTOR_SIZE - 1] = (ilev + 1 < NUM_LEV)
                                   ? elem.buffers.eta_dot_dpdn_buf(
                                         igp, jgp, ilev + 1)[0]
                                   : 0;
        // Add div_vdp before subtracting the previous value to eta_dot_dpdn
        // This will hopefully reduce numeric error
        tmp += elem.buffers.div_vdp(igp, jgp, ilev);
        tmp -= elem.buffers.eta_dot_dpdn_buf(igp, jgp, ilev);
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      for (int k = 1; k < NUM_PHYSICAL_LEV - 1; ++k) {
        ...
        facp = 0.5 *
               ...
               elem.buffers.eta_dot_dpdn_buf(igp, jgp,
                                                   ilevp1)[ivecp1];
        facm = 0.5 *
               (1 / elem.m_dp3d(m_data.n0, igp, jgp, ilev)[ivec]) *
               elem.buffers.eta_dot_dpdn_buf(igp, jgp, ilev)[ivec];
...
private:
  ...
  compute_pressure_impl(KernelVariables &kv, const Element& elem) const {
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
        ...
        auto p = elem.buffers.pressure(igp, jgp, ilev);
        ...
        elem.buffers.pressure(igp, jgp, ilev) = p;
      };
    });
    kv.team_barrier();
  }
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::single(Kokkos::PerThread(kv.team), [&]() {
        ...
        for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
          ...
          Real &p = elem.buffers.pressure(igp, jgp, ilev)[ivec];
  ...
  preq_hydrostatic_impl(KernelVariables &kv, const Element& elem) const {
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      for (int ilev = NUM_LEV - 1; ilev >= 0; --ilev) {
        ...
        const auto &p = elem.buffers.pressure(igp, jgp, ilev);
  ...
  preq_hydrostatic_impl(KernelVariables &kv, const Element& elem) const {
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      const auto rgas_tv_dp_over_p =
          Homme::subview(elem.buffers.vstar, 0, igp, jgp);
  ...
  preq_omega_ps_impl(KernelVariables &kv, const Element& elem) const {
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::single(Kokkos::PerThread(kv.team), [&]() {
        elem.buffers.omega_p(igp, jgp, 0) = 0;
        for (int ilev = 1; ilev < NUM_LEV; ++ilev) {
          elem.buffers.omega_p(igp, jgp, ilev) =
              elem.buffers.omega_p(igp, jgp, ilev - 1) +
              elem.buffers.div_vdp(igp, jgp, ilev - 1);
        }
      });
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
        ...
        elem.buffers.omega_p(igp, jgp, ilev) =
            (vgrad_p -
             (elem.buffers.omega_p(igp, jgp, ilev) +
              0.5 * elem.buffers.div_vdp(igp, jgp, ilev))) /
            p;
      });
    });
#ifdef DEBUG_TRACE
      ...
      Kokkos::single(Kokkos::PerThread(kv.team), [&]() {
        ...
        for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
          ...
          const Scalar vgrad_p =
              ...
              vn0(1, igp, jgp, ilev) *
                  elem.buffers.pressure_grad(1, igp, jgp, ilev);
          auto &omega_p = elem.buffers.omega_p(igp, jgp, ilev);
          const auto &p = elem.buffers.pressure(igp, jgp, ilev);
          const auto &div_vdp =
              elem.buffers.div_vdp(igp, jgp, ilev);

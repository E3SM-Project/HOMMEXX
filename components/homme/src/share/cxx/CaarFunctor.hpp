...
struct CaarFunctor {
  ...
  struct KernelVariables {
    ...
    static size_t shmem_size(int team_size) {
      size_t mem_size = (2*sizeof(Real[2][NP][NP]) + 2* sizeof(Real[NP][NP])) * team_size;
      return mem_size;
    }
  ...
  compute_energy_grad(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      // Ephi(igp, jgp) =
      //     0.5 * (m_region.U_current(kv.ie, kv.ilev)(igp, jgp)
      // *
      //                m_region.U_current(kv.ie, kv.ilev)(igp,
      // jgp) +
      //            m_region.V_current(kv.ie, kv.ilev)(igp, jgp)
      // *
      //                m_region.V_current(kv.ie, kv.ilev)(igp,
      // jgp)) +
      //     m_region.PHI(kv.ie)(kv.ilev, igp, jgp) +
      //     m_region.PECND(kv.ie, kv.ilev)(igp, jgp);
      kv.scalar_buf_1(igp, jgp) = m_region.U(kv.ie, m_data.n0, kv.ilev, igp, jgp) * m_region.U(kv.ie, m_data.n0, kv.ilev, igp, jgp) +
                                  m_region.V(kv.ie, m_data.n0, kv.ilev, igp, jgp) * m_region.V(kv.ie, m_data.n0, kv.ilev, igp, jgp);
      kv.scalar_buf_1(igp, jgp) *= 0.5;
      kv.scalar_buf_1(igp, jgp) += (m_region.PHI(kv.ie, kv.ilev, igp, jgp) + m_region.PECND(kv.ie, kv.ilev, igp, jgp));
      /*
       *kv.scalar_buf_1(igp, kv.jgp) +=
       *  m_region.PHI(kv.ie, kv.ilev, igp, kv.jgp);
       *kv.scalar_buf_1(kv.igp, kv.jgp) += m_region.PECND(kv.ie, kv.ilev, kv.igp, kv.jgp);
       */
    });

    gradient_sphere_update(kv.team, kv.scalar_buf_1, m_deriv.get_dvv(),
  ...
  void compute_velocity_np1(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, 2 * NP * NP),
      ...
      kv.vector_buf_2(hgp, igp, jgp) *= PhysicalConstants::Rgas*( m_region.get_3d_buffer(kv.ie,T_V,kv.ilev,igp,jgp)/p_ilev(igp,jgp) ) ;
      //kv.vector_buf_2(hgp, igp, jgp) *= m_scratch.T_v(
          //kv.ie, kv.ilev, igp, jgp);
      //kv.vector_buf_2(hgp, igp, jgp) /=
          //p_ilev(igp, jgp);
    });

    // kv.vector_buf_2 -> Ephi_grad + glnpsi
    compute_energy_grad(kv);

    const ExecViewUnmanaged<Real[NP][NP]> &vort = kv.scalar_buf_2;
    vorticity_sphere(kv.team,
                     m_region.U(kv.ie, m_data.n0, kv.ilev),
                     m_region.V(kv.ie, m_data.n0, kv.ilev), m_deriv.get_dvv(),
                     m_region.METDET(kv.ie), m_region.D(kv.ie),
                     kv.vector_buf_1, vort);
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      kv.vector_buf_2(0, igp, jgp) += /* v_vadv(igp, jgp) + */ m_region.V(kv.ie, m_data.n0, kv.ilev, igp, jgp) * vort(igp, jgp);
      kv.vector_buf_2(1, igp, jgp) *= -1;
      kv.vector_buf_2(1, igp, jgp) += /* v_vadv(igp, jgp) + */ - m_region.U(kv.ie, m_data.n0, kv.ilev, igp, jgp) * vort(igp, jgp);

      kv.vector_buf_2(0, igp, jgp) *= m_data.dt2;
      kv.vector_buf_2(0, igp, jgp) += m_region.U(kv.ie, m_data.nm1, kv.ilev, igp, jgp);
      kv.vector_buf_2(1, igp, jgp) *= m_data.dt2;
      kv.vector_buf_2(1, igp, jgp) += m_region.V(kv.ie, m_data.nm1, kv.ilev, igp, jgp);

      // Velocity at np1 = spheremp * buffer
      m_region.U(kv.ie, m_data.np1, kv.ilev, igp, jgp) = m_region.SPHEREMP(kv.ie, igp, jgp) * kv.vector_buf_2(0, igp, jgp);
      m_region.V(kv.ie, m_data.np1, kv.ilev, igp, jgp) = m_region.SPHEREMP(kv.ie, igp, jgp) * kv.vector_buf_2(1, igp, jgp);
    });
  }
  ...
  {
    const ExecViewUnmanaged<const Real[NUM_LEV][NP][NP]> pressure = m_region.get_3d_buffer(kv.ie,PRESSURE);
    const ExecViewUnmanaged<const Real[NUM_LEV][NP][NP]> T_v = m_region.get_3d_buffer(kv.ie,T_V);
    const ExecViewUnmanaged<const Real[NP][NP]> phis = m_region.PHIS(kv.ie);
    const ExecViewUnmanaged<const Real[NUM_LEV][NP][NP]> dp_n0 = m_region.DP3D(kv.ie, m_data.n0);
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      for (int ilev = NUM_LEV - 1; ilev >= 0; --ilev) {
        // compute phi
        phi(ilev, igp, jgp) = phis(igp, jgp) + kv.scalar_buf_1(igp,jgp) + PhysicalConstants::Rgas * T_v(ilev, igp, jgp) * (dp_n0(ilev, igp, jgp) * 0.5 / pressure(ilev, igp, jgp));

        // update phii
        kv.scalar_buf_1(igp, jgp) += PhysicalConstants::Rgas * T_v(ilev, igp, jgp) * 2.0 * (dp_n0(ilev, igp, jgp) * 0.5 / pressure(ilev, igp, jgp));
/*
 *        kv.vector_buf_2(0, igp, jgp) = PhysicalConstants::Rgas / pressure(kv.ilev, igp, jgp);
 *        kv.vector_buf_2(0, igp, jgp) *= T_v(kv.ilev, igp, jgp);
 *        kv.vector_buf_2(0, igp, jgp) *= dp_n0(kv.ilev, igp, jgp);
 *
 *        phi(kv.ilev, igp, jgp) = phis(igp, jgp) + kv.scalar_buf_1(igp, jgp);
 *        // FMA so no temporary register needed
 *        phi(kv.ilev, igp, jgp) += 0.5 * kv.vector_buf_2(0, igp, jgp);
 *
 *        kv.scalar_buf_1(igp, jgp) += kv.vector_buf_2(0, igp, jgp);
 */
      }
    });
  }
  ...
  void preq_omega_ps(KernelVariables &kv) const {
    ...
    for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
      ExecViewUnmanaged<const Real[NP][NP]> p_ilev = m_region.get_3d_buffer(kv.ie, PRESSURE, ilev);

      gradient_sphere(kv.team, p_ilev, m_deriv.get_dvv(),
                      m_region.DINV(kv.ie), kv.vector_buf_2, grad_p);

      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
                           [&](const int loop_idx) {
        const int igp = loop_idx / NP;
        const int jgp = loop_idx % NP;
        Real vgrad_p = m_region.U(kv.ie, m_data.n0, ilev, igp, jgp) * grad_p(0, igp, jgp)
                     + m_region.V(kv.ie, m_data.n0, ilev, igp, jgp) * grad_p(1, igp, jgp);
        /*
        ...
        m_region.get_3d_buffer(kv.ie, OMEGA_P, ilev, igp, jgp) = vgrad_p / p_ilev(igp, jgp);

////////////////////////
        m_region.get_3d_buffer(kv.ie, OMEGA_P, ilev, igp, jgp) -= ( 1.0 / p_ilev(igp, jgp) * integral(igp, jgp) + (0.5/ p_ilev(igp, jgp))
                                                                *  m_region.get_3d_buffer(kv.ie, DIV_VDP, ilev, igp, jgp) );
////////////////////////
        integral(igp, jgp) += m_region.get_3d_buffer(kv.ie, DIV_VDP, ilev, igp, jgp);
      });
    }
  }
  ...
  void compute_pressure(KernelVariables &kv) const {
    ExecViewUnmanaged<Real[NUM_LEV][NP][NP]> pressure = m_region.get_3d_buffer(kv.ie, PRESSURE);
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
                         [&](const int idx) {
      const int igp = idx / NP;
      const int jgp = idx % NP;
  ...
  void compute_temperature_no_tracers_helper(KernelVariables &kv) const {
    ExecViewUnmanaged<Real[NP][NP]> T_v = m_region.get_3d_buffer(kv.ie, T_V, kv.ilev);
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
                         [&](const int idx) {
      const int igp = idx / NP;
      const int jgp = idx % NP;
  ...
  void compute_temperature_tracers_helper(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      Real &Qt = kv.temp[0];
      Qt = m_region.QDP(kv.ie, m_data.qn0, 0, kv.ilev, igp, jgp) /
  ...
  void compute_div_vdp(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      kv.vector_buf_2(0,igp,jgp) = m_region.U(kv.ie,m_data.n0,kv.ilev,igp,jgp) * m_region.DP3D(kv.ie,m_data.n0,kv.ilev,igp,jgp);
      kv.vector_buf_2(1,igp,jgp) = m_region.V(kv.ie,m_data.n0,kv.ilev,igp,jgp) * m_region.DP3D(kv.ie,m_data.n0,kv.ilev,igp,jgp);

      kv.scalar_buf_1(igp,jgp) = m_data.eta_ave_w * kv.vector_buf_2(0, igp, jgp);
      m_region.DERIVED_UN0(kv.ie,kv.ilev,igp,jgp) = m_region.DERIVED_UN0(kv.ie,kv.ilev,igp,jgp) + kv.scalar_buf_1(igp, jgp);

      kv.scalar_buf_1(igp, jgp) = m_data.eta_ave_w * kv.vector_buf_2(1,igp,jgp);
      m_region.DERIVED_VN0(kv.ie, kv.ilev,igp, jgp) = m_region.DERIVED_VN0(kv.ie, kv.ilev, igp, jgp) + kv.scalar_buf_1(igp, jgp);
    });

    divergence_sphere(kv.team, kv.vector_buf_2, m_deriv.get_dvv(),
                      m_region.METDET(kv.ie), m_region.DINV(kv.ie),
                      kv.vector_buf_1, m_region.get_3d_buffer(kv.ie,DIV_VDP,kv.ilev) );
  ...
  void compute_omega_p(KernelVariables &kv) const {
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      const int igp = idx / NP;
      const int jgp = idx % NP;
      m_region.OMEGA_P(kv.ie, kv.ilev, igp, jgp) += m_data.eta_ave_w * m_region.get_3d_buffer(kv.ie, OMEGA_P, kv.ilev, igp, jgp);
    });
  }
  ...
  void compute_temperature_np1(KernelVariables &kv) const {
    ...
    gradient_sphere(kv.team, temperature, m_deriv.get_dvv(),
                    m_region.DINV(kv.ie), kv.vector_buf_2,
                    grad_tmp);
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      Real &vgrad_t = kv.temp[0];
      vgrad_t = m_region.U(kv.ie,m_data.n0,kv.ilev,igp,jgp) * grad_tmp(0,igp,jgp)
              + m_region.V(kv.ie,m_data.n0,kv.ilev,igp,jgp) * grad_tmp(1,igp,jgp);

      // vgrad_t + kappa * T_v * omega_p
      Real &ttens = kv.temp[0];
      ttens = -vgrad_t + PhysicalConstants::kappa * m_region.get_3d_buffer(kv.ie,T_V,kv.ilev,igp,jgp) * m_region.get_3d_buffer(kv.ie,OMEGA_P,kv.ilev,igp,jgp);

      Real &temp_np1 = kv.temp[0];
      temp_np1  = ttens * m_data.dt2 + m_region.T(kv.ie,m_data.nm1,kv.ilev,igp,jgp);
  ...
  void compute_dp3d_np1(KernelVariables &kv) const {
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      Real tmp = m_region.DP3D(kv.ie, m_data.nm1, kv.ilev, igp, jgp);
           tmp-= m_data.dt2 * m_region.get_3d_buffer(kv.ie, DIV_VDP, kv.ilev, igp, jgp);
      m_region.DP3D(kv.ie, m_data.np1, kv.ilev, igp, jgp) = m_region.SPHEREMP(kv.ie, igp, jgp) * tmp;
    });
  }

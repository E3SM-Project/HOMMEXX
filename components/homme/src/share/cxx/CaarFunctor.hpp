...
struct CaarFunctor {
  ...
  CaarFunctor(const CaarControl &data)
      : m_data(data), m_region(get_region()), m_scratch(data.num_elems),
  ...
  KOKKOS_INLINE_FUNCTION void compute_energy_grad(KernelVariables &kv) const {
      ...
      Ephi(igp, jgp) =
          0.5 * (m_region.U(kv.ie, m_data.n0, kv.ilev, igp, jgp) *
                 ...
                 m_region.V(kv.ie, m_data.n0, kv.ilev, igp, jgp) *
                     m_region.V(kv.ie, m_data.n0, kv.ilev, igp, jgp));
      Ephi(igp, jgp) += m_region.PHI(kv.ie, kv.ilev, igp, jgp);
      Ephi(igp, jgp) += m_region.PECND(kv.ie, kv.ilev, igp, jgp);
    });
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, 2 * NP * NP),
      ...
      gpterm(hgp, igp, jgp) *=
          ...
          (m_scratch.T_v(kv.ie, kv.ilev, igp, jgp) / p_ilev(igp, jgp));
    });
  ...
  void compute_velocity_np1(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      kv.vector_buf_2(0, igp, jgp) += /* v_vadv(igp, jgp) + */ m_region.V(
                                          kv.ie, m_data.n0, kv.ilev, igp, jgp) *
                                      vort(igp, jgp);
      kv.vector_buf_2(1, igp, jgp) *= -1;
      kv.vector_buf_2(1, igp, jgp) +=
          /* v_vadv(igp, jgp) + */ -m_region.U(kv.ie, m_data.n0, kv.ilev, igp,
                                               jgp) *
          vort(igp, jgp);
  ...
  void preq_omega_ps(KernelVariables &kv) const {
    ...
    for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
        ...
        const int jgp = loop_idx % NP;
        Real vgrad_p =
            m_region.U(kv.ie, m_data.n0, ilev, igp, jgp) * grad_p(0, igp, jgp) +
        ...
        ////////////////////////
        m_scratch.omega_p(kv.ie, ilev, igp, jgp) -=
            (1.0 / p_ilev(igp, jgp) * integral(igp, jgp) +
             (0.5 / p_ilev(igp, jgp)) *
                 m_scratch.div_vdp(kv.ie, ilev, igp, jgp));
        ////////////////////////
  ...
  void compute_pressure(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      // TODO: change the sum into p(k) = p(k-1) + 0.5*( dp(k)+dp(k-1) ) to
      // increase accuracy
  ...
  void compute_temperature_no_tracers_helper(KernelVariables &kv) const {
    ExecViewUnmanaged<Real[NUM_LEV][NP][NP]> T_v =
        m_scratch.T_v(kv.team.league_rank());
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
                         [&](const int idx) {
      const int igp = idx / NP;
      const int jgp = idx % NP;
  ...
  void compute_temperature_tracers_helper(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      Qt += 1.0;
      T_v(kv.ilev, igp, jgp) =
          m_region.T(kv.ie, m_data.n0, kv.ilev, igp, jgp) * Qt;
    });
  }
  ...
  void compute_omega_p(KernelVariables &kv) const {
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      m_region.OMEGA_P(kv.ie, kv.ilev, igp, jgp) +=
          m_data.eta_ave_w * m_scratch.omega_p(kv.ie, kv.ilev, igp, jgp);
    });
  }
  ...
  void compute_temperature_np1(KernelVariables &kv) const {
    ...
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NP * NP),
      ...
      Real ttens = -vgrad_t + PhysicalConstants::kappa *
                                  m_scratch.T_v(kv.ie, kv.ilev, igp, jgp) *
                                  m_scratch.omega_p(kv.ie, kv.ilev, igp, jgp);

      Real temp_np1 =
          ttens * m_data.dt2 + m_region.T(kv.ie, m_data.nm1, kv.ilev, igp, jgp);

...
public:
  ...
  SphereOperators (const int num_elems, const int qsize = 1)
    ...
    , scalar_buf_2    ("scalar buffer 2", nthreads)
    , scalar_buf_3    ("scalar buffer 3", nthreads)
    , vector_buf_1    ("vector buffer 1", nthreads)
    , vector_buf_2    ("vector buffer 2", nthreads)
    , vector_buf_3    ("vector buffer 3", nthreads)
  {
...
// ================ SINGLE-LEVEL IMPLEMENTATION =========================== //
                      ...
                      const ExecViewUnmanaged<      Real [2][NP][NP]> grad_s) const
  {
    ...
    const auto& temp_v_buf = Homme::subview(vector_buf_sl_1,id(kv));
                        ...
                        const ExecViewUnmanaged<      Real    [NP][NP]> div_v) const
  {
    ...
    const auto& D_inv = Homme::subview(m_dinv,kv.ie);
    const auto& gv_buf = Homme::subview(vector_buf_sl_1,id(kv));
    constexpr int np_squared = NP * NP;
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, np_squared),
                         [&](const int loop_idx) {
                       ...
                       const ExecViewUnmanaged<      Real [NP][NP]> vort) const
  {
    ...
    const auto& metdet = Homme::subview(m_metdet,kv.ie);
    const auto& vcov_buf = Homme::subview(vector_buf_sl_1,id(kv));
  ...
  {
    const auto& grad_s = Homme::subview(vector_buf_sl_2, id(kv));
                   ...
                   const ExecViewUnmanaged<      Scalar [2][NP][NP][NUM_LEV]> grad_s) const
  {
    ...
    const auto& D_inv = Homme::subview(m_dinv, kv.ie);
    const auto& v_buf = Homme::subview(vector_buf_1,id(kv));
                     ...
                     const ExecViewUnmanaged<      Scalar    [NP][NP][NUM_LEV]> div_v) const
  {
    ...
    const auto& D_inv = Homme::subview(m_dinv, kv.ie);
    const auto& metdet = Homme::subview(m_metdet, kv.ie);
    const auto& gv_buf = Homme::subview(vector_buf_1,id(kv));
    constexpr int np_squared = NP * NP;
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, np_squared),
                         [&](const int loop_idx) {
      const int igp = loop_idx / NP;
                            ...
                            const ExecViewUnmanaged<      Scalar    [NP][NP][NUM_LEV]> div_v) const
  {
    ...
    const auto& D_inv = Homme::subview(m_dinv, kv.ie);
    const auto& metdet = Homme::subview(m_metdet, kv.ie);
    const auto& gv = Homme::subview(vector_buf_1,id(kv));
    constexpr int np_squared = NP * NP;
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, np_squared),
                         [&](const int loop_idx) {
      const int igp = loop_idx / NP;
                    ...
                    const ExecViewUnmanaged<      Scalar [NP][NP][NUM_LEV]> vort) const
  {
    ...
    const auto& D = Homme::subview(m_d, kv.ie);
    const auto& metdet = Homme::subview(m_metdet, kv.ie);
    const auto& vcov_buf = Homme::subview(vector_buf_1,id(kv));
    constexpr int np_squared = NP * NP;
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, np_squared),
                         [&](const int loop_idx) {
      const int igp = loop_idx / NP;
                    ...
                    const ExecViewUnmanaged<      Scalar    [NP][NP][NUM_LEV]> vort) const
  {
    ...
    const auto& D = Homme::subview(m_d, kv.ie);
    const auto& metdet = Homme::subview(m_metdet, kv.ie);
    const auto& sphere_buf = Homme::subview(vector_buf_1,id(kv));
    constexpr int np_squared = NP * NP;
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, np_squared),
                         [&](const int loop_idx) {
      const int igp = loop_idx / NP;
                  ...
                  const ExecViewUnmanaged<      Scalar [NP][NP][NUM_LEV]> laplace) const
  {
    ...
    const auto& grad_s = Homme::subview(vector_buf_2, id(kv));
      // let's ignore var coef and tensor hv
                 ...
                 const ExecViewUnmanaged<      Scalar       [NP][NP][NUM_LEV]> laplace) const
  {
    ...
    const auto& grad_s = Homme::subview(vector_buf_2, id(kv));
    const auto& sphere_buf = Homme::subview(vector_buf_3, id(kv));
    const auto& D_inv = Homme::subview(m_dinv, kv.ie);
    const auto& spheremp = Homme::subview(m_spheremp, kv.ie);
                                 ...
                                 const ExecViewUnmanaged<      Scalar    [2][NP][NP][NUM_LEV]> curls) const
  {
    ...
    const auto& D = Homme::subview(m_d, kv.ie);
    const auto& mp = Homme::subview(m_mp, kv.ie);
    const auto& sphere_buf = Homme::subview(vector_buf_2,id(kv));
    constexpr int np_squared = NP * NP;
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
      const int ngp = loop_idx / NP;
      const int mgp = loop_idx % NP;
                                ...
                                const ExecViewUnmanaged<      Scalar    [2][NP][NP][NUM_LEV]> laplace) const
  {
    ...
    const auto& D_inv = Homme::subview(m_dinv, kv.ie);
    const auto& spheremp = Homme::subview(m_spheremp, kv.ie);
    const auto& grads      = Homme::subview(vector_buf_1,id(kv));
    const auto& laplace0   = Homme::subview(scalar_buf_1,id(kv));
    const auto& laplace1   = Homme::subview(scalar_buf_2,id(kv));
    const auto& laplace2   = Homme::subview(scalar_buf_3,id(kv));
    const auto& sphere_buf = Homme::subview(vector_buf_2,id(kv));
    constexpr int np_squared = NP * NP;
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, np_squared),
                         [&](const int loop_idx) {
...
#define UNDAMPRRCART
                             ...
                             const ExecViewUnmanaged<      Scalar [2][NP][NP][NUM_LEV]> laplace) const
  {
    ...
    const auto& D = Homme::subview(m_d, kv.ie);
    const auto& spheremp = Homme::subview(m_spheremp, kv.ie);
    const auto& div           = Homme::subview(scalar_buf_2,id(kv));
    const auto& vort          = Homme::subview(scalar_buf_2,id(kv));
    const auto& grad_curl_cov = Homme::subview(vector_buf_3,id(kv));
    constexpr int np_squared = NP * NP;

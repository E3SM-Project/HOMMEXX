                ...
                ExecViewUnmanaged<Real[2][NP][NP]> grad_s)
{
  ...
  Kokkos::parallel_for(
      Kokkos::ThreadVectorRange(kv.team, grad_iters),
      [&](const int loop_idx) {
                  ...
                  ExecViewUnmanaged<Real[NP][NP]> div_v)
{
  ...
  Kokkos::parallel_for(
      ...
      [&](const int loop_idx) {
        ...
        gv[hgp][jgp][igp] = region.m_dinv(kv.ie, hgp, 0, jgp, igp) * v(0, jgp, igp)
  ...
  Kokkos::parallel_for(
      Kokkos::ThreadVectorRange(kv.team, div_iters),
      [&](const int loop_idx) {
        ...
        div_v(jgp, igp) = (dudx + dvdy) * ((1.0 / region.m_metdet(kv.ie, jgp, igp)) * PhysicalConstants::rrearth);
      }
  );
}
                 ...
                 ExecViewUnmanaged<Real[NP][NP]> vort)
{
  ...
  Real vcov[2][NP][NP];
  Kokkos::parallel_for(
      Kokkos::ThreadVectorRange(kv.team, covar_iters),
      [&](const int loop_idx) {
        const int igp = loop_idx / NP;
        const int jgp = loop_idx % NP;
  ...
  Kokkos::parallel_for(
      Kokkos::ThreadVectorRange(kv.team, vort_iters),
      [&](const int loop_idx) {
        const int igp = loop_idx / NP;
        const int jgp = loop_idx % NP;
        Real dudy = 0.0;
        ...
        vort(jgp, igp) = (dvdx - dudy) * ((1.0 / region.m_metdet(kv.ie, jgp, igp)) * PhysicalConstants::rrearth);
      }
  );
}

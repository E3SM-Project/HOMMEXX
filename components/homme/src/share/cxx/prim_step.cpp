...
void prim_advance_exp_c(const bool);
void prim_advec_tracers_remap_c();

void prim_step_c (const bool compute_diagnostics)
{
  ...
  auto vstar = elements.m_derived_vstar;
  auto state_u = elements.m_u;
  auto state_v = elements.m_v;
  auto dp = elements.m_derived_dp;
  auto state_dp3d = elements.m_dp3d;
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team, NP * NP),
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
        ...
        if (data.use_semi_lagrangian_transport) {
          vstar(ie,0,igp,jgp,ilev) = state_u(ie,tl.n0,igp,jgp,ilev);
          vstar(ie,1,igp,jgp,ilev) = state_v(ie,tl.n0,igp,jgp,ilev);
        }
        dp(ie,igp,jgp,ilev) = state_dp3d(ie,tl.n0,igp,jgp,ilev);
      });
    });
  });
  ...
  // Dynamical Step
  // ===============
  prim_advance_exp_c(compute_diagnostics);

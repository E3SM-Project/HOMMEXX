...
struct VerticalRemapManager::Impl {
  Impl (const Control& c, const Elements& e, RemapAlg alg) {
    if (alg == RemapAlg::PPM_FIXED) {
      if (c.rsplit != 0) {
        remapper = std::make_shared<RemapFunctor<true, PpmVertRemap, PpmFixed> >(c, e);
      } else {
        remapper = std::make_shared<RemapFunctor<false, PpmVertRemap, PpmFixed> >(c, e);
      }
    } else if (alg == RemapAlg::PPM_MIRRORED) {
      if (c.rsplit != 0) {
        remapper = std::make_shared<RemapFunctor<true, PpmVertRemap, PpmMirrored> >(c, e);
      } else {
        remapper = std::make_shared<RemapFunctor<false, PpmVertRemap, PpmMirrored> >(c, e);
      }
    } else {
...
::VerticalRemapManager () {
  ...
  assert(p.params_set);
  p_.reset(new Impl(c, e, p.remap_alg));
}

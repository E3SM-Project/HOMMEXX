...
struct VerticalRemapManager::Impl {
  Impl(const SimulationParams &params, const Elements &e, const Tracers &t, const HybridVCoord &h) {
    if (params.remap_alg == RemapAlg::PPM_FIXED) {
      if (params.rsplit != 0) {
        remapper = std::make_shared<Remap::RemapFunctor<
            true, Remap::Ppm::PpmVertRemap, Remap::Ppm::PpmFixed> >(params.qsize, e, t, h);
      } else {
        remapper = std::make_shared<Remap::RemapFunctor<
            false, Remap::Ppm::PpmVertRemap, Remap::Ppm::PpmFixed> >(params.qsize, e, t, h);
      }
    } else if (params.remap_alg == RemapAlg::PPM_MIRRORED) {
      if (params.rsplit != 0) {
        remapper = std::make_shared<Remap::RemapFunctor<
            true, Remap::Ppm::PpmVertRemap, Remap::Ppm::PpmMirrored> >(params.qsize, e, t, h);
      } else {
        remapper = std::make_shared<Remap::RemapFunctor<
            false, Remap::Ppm::PpmVertRemap, Remap::Ppm::PpmMirrored> >(params.qsize, e, t, h);
      }
    } else {

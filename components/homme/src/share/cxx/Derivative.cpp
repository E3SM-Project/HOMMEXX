...
Derivative::Derivative()
    : m_dvv_exec("dvv"), m_integ_mat_exec("integration matrix"),
...
void Derivative::init(CF90Ptr &dvv_ptr, CF90Ptr & /*integration_mat_ptr*/,
                      CF90Ptr & /*bd_interpolation_mat_ptr*/) {
  ...
  // flip_f90_array_3d_213<NC,2,NP> (bd_interpolation_mat_ptr,
  // bd_interp_mat_host);
...
void Derivative::random_init(std::mt19937_64 &engine) {
  ...
  ExecViewManaged<Real[NC][NP]>::HostMirror integ_mat_host =
      Kokkos::create_mirror_view(m_integ_mat_exec);
  ExecViewManaged<Real[2][NC][NP]>::HostMirror bd_interp_mat_host =
      Kokkos::create_mirror_view(m_bd_interp_mat_exec);
...
void subcell_div_fluxes(
  ...
  // Helpers
  ExecViewUnmanaged<const Real[NC][NP]> integ_mat =
      deriv.get_integration_matrix();

  ExecViewUnmanaged<const Real[2][NC][NP]> bd_interp_mat =
      deriv.get_bd_interpolation_matrix();


  ExecViewUnmanaged<const Real[NC][NP]> bd_interp_mat_0 =
      subview(bd_interp_mat, 0, ALL(), ALL());


  ExecViewUnmanaged<const Real[NC][NP]> bd_interp_mat_1 =
      subview(bd_interp_mat, 1, ALL(), ALL());
  ...
  Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, NP * NP),
  ...
  matrix_matrix<NC, NP, NP, NP, false, true>(team_member, tb, bd_interp_mat_0,
                                             flux_b);
  matrix_matrix<NC, NP, NP, NP, false, true>(team_member, tb, bd_interp_mat_1,
                                             flux_t);

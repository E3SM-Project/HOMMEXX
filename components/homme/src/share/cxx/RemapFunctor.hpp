...
struct RemapFunctor : public Remapper,
  ...
  explicit RemapFunctor(const int qsize, const Elements &elements, const Tracers &tracers,
                        const HybridVCoord& hvcoord)
      : _RemapFunctorRSplit<nonzero_rsplit>(elements.num_elems()),
        m_data(qsize), m_elements(elements), m_tracers(tracers), m_hvcoord(hvcoord),

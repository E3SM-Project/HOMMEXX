...
public:
  ...
  EulerStepFunctorImpl ()
   ...
   , m_t          (Context::singleton().get_tracers())
  ...
  void init_boundary_exchanges () {
...

    {
      ...
      m_mmqb_be->register_field(m_t.qtens_biharmonic, m_data.qsize, 0);
    ...
    for (int np1_qdp = 0, k = 0; np1_qdp < Q_NUM_TIME_LEVELS; ++np1_qdp) {
      for (int dssi = 0; dssi < 3; ++dssi, ++k) {
        ...
        be.register_field(m_t.m_qdp, np1_qdp, m_data.qsize, 0);
  ...
  void operator() (const BIHPre&, const TeamMember& team) const {
    ...
    const auto qtens_biharmonic = Homme::subview(m_t.qtens_biharmonic, kv.ie, kv.iq);
    if (m_data.nu_p > 0) {

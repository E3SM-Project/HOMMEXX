...
class trilinosModelEvaluator
    : public EpetraExt::ModelEvaluator {
 public:
  // 1. Constructors. Different interfaces for different
  // preconditioners

  /* Identity Preconditioner interface */
  trilinosModelEvaluator(
      int nelems, double *statevector_,
      const Epetra_Comm &comm_, void *blackbox_res,
      void *precdata, void *jacdata,
  ...
  RCP<const Teuchos::Array<std::string> > get_p_names(
      int l) const;
  ...
  //! Reset State
  //    void ResetState(double *statevector,void*
  //    blackbox_res_);

  //! Evaluate model on InArgs
  void evalModel(const InArgs &inArgs,
                 const OutArgs &outArgs) const;
  //@}

  // 3. Extra interface functions specific to this
  // implementation
  ...
  // 4. Utility functions not accessible from outside this
  // class.
 private:
 ...
 private:
  ...
  void (*auxprecFunctionblock12)(double *, int, double *,
                                 void *);
  void (*auxprecFunctionblock21)(double *, int, double *,
                                 void *);
...
class hommeJacobianVectorProduct : public Epetra_Operator {
 public:
  ...
  // Constructor, called once per time step (or maybe once
  // per run)
  hommeJacobianVectorProduct(
      int N, RCP<Epetra_Vector> xVec, RCP<Epetra_Map> xMap,
      void *jacdata,
      void (*jacFunction)(double *, int, double *, void *),
      void (*jacUpdateFunction)(double *, int, void *));

  // compute Jacobian called once per Newton step
  virtual int computeJacobian(RCP<const Epetra_Vector> xVec,
                              void *precdata_);
  // Apply Jacobian called once per Krylov iter
  virtual int Apply(const Epetra_MultiVector &V,
                    Epetra_MultiVector &Y) const;

  // Trivial implemetations of other required functions set
  // here
  int SetUseTranspose(bool UseTranspose) {
    TEUCHOS_TEST_FOR_EXCEPT(UseTranspose);
    return 0;
  };
  ...
  double NormInf() const {
    throw "NO NormInf Implemented in JacobianVectorProduct";
  };
  const char *Label() const {
    return "JacobianVectorProduct";
  };
...
class hommePreconditionerBase : public Epetra_Operator {
 public:
  ...
  hommePreconditionerBase(RCP<Epetra_Map> xMap)
      : xMap_(xMap){};

  // Methods that MUST be implemented for preconditioner
  virtual int computePreconditioner(
      RCP<const Epetra_Vector> xVec, void *precdata_) = 0;
  ...
  double NormInf() const {
    throw "NO NormInf Implemented in trilinosPrecon";
  };
 ...
 private:
  hommePreconditionerBase();  // NOT ACCESSIBLE, NEED TO USE
                              // OTHER CONSTRUCTOR};
...
/*******************************************************************************/
/*******************************************************************************/
// Add Preconditioners, inheriting from
// hommePreconditionerBase to get
// default implementations of most required methods of an
// Epetra_Operator
// above. Only need to implement constructor,
...
class identityPreconditioner
    : public hommePreconditionerBase {
 public:
...
class simplePreconditioner
 ...
 public:
  simplePreconditioner(
      ...
      const RCP<ParameterList> &FSolvePL_,
      const RCP<ParameterList> &SchurSolvePL_,
      int *FTotalIt_, int *SchurTotalIt_);
 ...
 private:
  ...
  Teuchos::RCP<Belos::SolverManager<ST, MV, OP> >
      SchurSolver;
  ...
  Teuchos::RCP<Belos::LinearProblem<ST, MV, OP> >
      SchurProblem;
...
class simpleMLPreconditioner
 ...
 public:
  ...
  int computePreconditioner(RCP<const Epetra_Vector> xVec,
                            void *precdata_);
  // void BuildMaps(int* nelems, int* nets, int* nete, int*
  // np, int* nlev, void* data);
 ...
 private:
  ...
  bool StaticProfile;
  int NPPP;  // Number of Points Per Processor (unique to
             // processor but not across processors)
};
#endif

...
void sw_jacobian(double *, int, double *,
                 void *);  // jacobian-vector product
...
void sw_picard_block_21(double *, int, double *, int,
                        void *);
void sw_picard_DFinvBt(double *, int, double *, int,
                       void *);
...
RCP<Teuchos::ParameterList> readParameterListAndSetDefaults(
    MPI_Comm &mpi_comm_c);

// void printNewtonKrylovFinalStats(Teuchos::ParameterList&
// nlParams,
...
/*Convention: for SIMPLE algorithm based on Picard
  ...
  void (*precFunctionblock12)(double *, int, double *, int,
                              void *) = sw_picard_DFinvBt;
  void (*precFunctionblock21)(double *, int, double *, int,
  ...
  try {
    ...
    //  AGS: MPI_Comm_f2c did not work on my machine -- all
    //  procs were rank=0
    ...
    if(printProc)
      cout << "NOXINIT CALLED    for nelem=" << N
           << ". Looking for trilinosOptions.xml" << endl;

    try {
      paramList =
          readParameterListAndSetDefaults(mpi_comm_c);
    ...
    model = Teuchos::rcp(new trilinosModelEvaluator(
        N, statevector, Comm, blackbox_res, blackbox_prec,
        jac_data, residualFunction, jacFunction,
        jacUpdateFunction, precUpdateFunction,
        getJacVector));
#endif
    ...
    model = Teuchos::rcp(new trilinosModelEvaluator(
        N, statevector, Comm, blackbox_res, blackbox_prec,
        jac_data, residualFunction, precFunctionblock11,
        precFunctionblock12, precFunctionblock21,
        precFunctionblock22, jacFunction, jacUpdateFunction,
        precUpdateFunction, getJacVector, FSolvePL,
        SchurSolvePL, FTotalIt, SchurTotalIt));
#endif
    ...
    model = Teuchos::rcp(new trilinosModelEvaluator(
        ...
        precFunctionblock22, jacFunction, jacUpdateFunction,
        precUpdateFunction, getJacVector, FSolvePL,
        SchurSolvePL, FTotalIt, SchurTotalIt, get_globalIDs,
        get_HelmElementMat, get_HelmMap, HelmSolvePL,
        HelmTotalIt, nets, nete, np, nlev));
#endif

    Nsolver =
        rcp(new Piro::Epetra::NOXSolver(paramList, model));
    ...
    // Time step counter: just for deciding whether to use
    // continuation on relaxatin param
    timeStep++;
...
void noxsolve(int *nelems, double *statevector,
              ...
              void *jac_data, int *ierr) {
  *ierr = 0;
  try {
    TEUCHOS_TEST_FOR_EXCEPTION(
        is_null(Nsolver), logic_error,
    ...
    if(xout == Teuchos::null)
      throw "evalModel is NOT returning a vector";

    for(int i = 0; i < *nelems; i++)
      statevector[i] = (*xout)[i];
  } catch(std::exception &e) {
    cout << e.what() << endl;
  } catch(const char *s) {
    cout << s << endl;
...
RCP<Teuchos::ParameterList> readParameterListAndSetDefaults(
  ...
  RCP<Teuchos::ParameterList> params =
      rcp(new Teuchos::ParameterList(
          "Trilinos Options for Piro"));

  Teuchos::MpiComm<int> tcomm(
      Teuchos::opaqueWrapper(mpi_comm_c));
  Teuchos::updateParametersFromXmlFileAndBroadcast(
      "trilinosOptions.xml", params.ptr(), tcomm);

  // Set 2 defaults different then NOX defaults unless
  // overwridden in input file
  ...
  Teuchos::ParameterList &nlPrintParams =
      params->sublist("NOX").sublist("Printing");
  ...
  validPL.sublist("FSolvePL");
  validPL.sublist("SchurSolvePL");
...
// Andy this is the function that I am trying to write to
// accumulate the total newton iteration and the total
// linear iteration
...
/*
  ...
  int NewtonIters =
  nlParams.sublist("Output").get("Nonlinear Iterations",
  -1000);
  ...
  stepNum++;
  if (stepNum > 1) {
  //make this 120 parameter something read in from file
  based on IO frequency
  //if (stepNum == 120) {
  ...
  << "Average Newton Steps per timestep" << (double)
  totalNewtonIters / (double) stepNum<<endl
  << "Average Linear Steps per Newton Step" << (double)
  KrylovIters / (double) totalNewtonIters <<endl
  << "Average Linear Steps per Time Step" << (double)
  KrylovIters / (double) stepNum<<endl;
  //}
  }
  }
*/

...

{
 public:
  ...
  /* interface for comparing two block preconditioner
   * formulations */
  Problem_Interface(
      int nelems, double *statevector_,
      const LOCA::ParameterVector &pVector_,
      ...
      void (*precFunctionblock22)(double *, int, double *,
                                  void *),
      void (*auxprecFunctionblock11)(double *, int,
                                     double *, void *),
      void (*auxprecFunctionblock12)(double *, int,
                                     double *, void *),
      void (*auxprecFunctionblock21)(double *, int,
                                     double *, void *),
      void (*auxprecFunctionblock22)(double *, int,
                                     double *, void *),
      void (*jacFunction)(double *, int, double *, void *),
      void (*precUpdateFunction)(double *, int, void *),
      void (*getJacVector)(double *, int, void *));
  ...
  */
  ...
  virtual void printSolution(const Epetra_Vector &x,
                             double conParam);

  //! Application Operator: Object that points to the user's
  //! evaluation routines.
  /*! This is used to point to the actual routines and to
   * store
   *  auxiliary data required by the user's application for
   * function/Jacobian
   *  evaluations that NOX does not need to know about.
   * This is type of
  ...
  int SetUseTranspose(bool UseTranspose) {
    if(UseTranspose) {
      cerr << "ERROR: No noxlocainterface::SetUseTranspose"
           << endl;
      return -1;
    } else
      return 0;
  ...
  double NormInf() const {
    cerr << "ERROR: No noxlocainterface::Apply" << endl;
    return 1.0;
  };
  // const char* Label() const { return
  // "noxlocainterface::user preconditioner";};
  ...
  //                 PrePostOperator (const Teuchos::RCP<
  //                 NOX::Utils > &utils,
  //                 Teuchos::ParameterList
  //                 &solverOptionsSubList);
  //  virtual ~PrePostOperator ();

  virtual void runPreIterate(

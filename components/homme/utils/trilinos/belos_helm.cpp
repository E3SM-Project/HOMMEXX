...
// allow Belos to be called in 3 stages (init, solve,
// finalize)
...
static Teuchos::RCP<ML_Epetra::MultiLevelPreconditioner>
    MLPrec;
...
static Teuchos::RCP<Belos::LinearProblem<ST, MV, OP> >
    HelmProblem;
static Teuchos::RCP<Belos::SolverManager<ST, MV, OP> >
    HelmSolver;
...
static int N;  // number of points on each processor (unique
               // to processor but not across processors)
...
/*
This routine builds the maps for the Helmholtz Matrix
assembly and rhs assembly

the data from HOMME is configured so that DOF's can be
represented multiple times
we first remove these multiplities and record the indices of
the vector corresponding
to the unique index in the "bin" structure. A point can be
represented at most on 4 non-unique indices
the first index of bin[i] stores the unique index on the
local processor, the second index stores the
non-index indices. bincount[i] counts the number of bins
(multiplicties) for a given in index. The local rhs vector
will need to be scaled by this number in the assembly
process.

The second level assembly is also performed to remove
non-unique entries across processors. This is done using the
ParMatrixMap and the inporter/exporter between the
ParMatrixMap and the local MatrixMap

*/
void BuildMaps(int *nelems, int *nets, int *nete, int *np,
               int *nlev, void *data) {
  ...
  try {
    ...
    std::sort(&my_GlobalIDs[0],
              &my_GlobalIDs[0] + (*nelems));

    int *end =
        unique(my_GlobalIDs, &my_GlobalIDs[0] + (*nelems));
    ...
    for(int i = 0; i < N; i++) {
      for(int j = 0; j < 5; j++) {
        bin[i][j] =
            *nelems +
            1;  // initialize to out index out of bounds
      }
    }
    ...
    MatrixMap = Teuchos::rcp(
        new Epetra_Map(-1, N, my_GlobalIDs, 0, *Comm));
    ...
    cout << "Caught unknown exception in BuildMaps "
         << endl;
  }
}  // end BuildMaps

void BuildMatrix(int *nelems, int *nets, int *nete, int *np,
                 int *nlev, void *data) {
  ...
  try {
    ...
    const std::size_t approximate_indices_per_row =
        (*nelems);
...
void BuildRHS(int *nets, int *nete, int *np, int *nlev,
              double *rhsVector, void *data) {

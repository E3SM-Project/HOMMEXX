...
// Aaron! duplicate routine above for GMRES only
// preconditioner
//#include "precon_interface_gmres.hpp" // Interface file to
//NOX

// Required for reading and writing parameter lists from xml
// format
...
// allow NOX to be called in 3 stages (init, solve,
// finalize)
...
// pal Comm needs to be defined in this scope to be passed
// to preconditioner objects
static Teuchos::RCP<Epetra_MpiComm> Comm;
// pal print proc used for debug printing
static bool printproc;

// Define corresponding variables for NOX to be used by
// precon
...
/*Defining external functions for finite difference
 * Jacobian*/
...
/*Defining external functions for analytic Picard
 * linearization*/
void sw_picard(double *, int, double *, void *);
/*Defining external functions for applying preconditioners
 * and preconditioner blocks*/
...
void sw_picard_block_FDiagInv(double *, int, double *,
                              void *);
...
//  void precon_wgmres(double *, double *, int, double*,
//  void *, void *);
// when you create this, the bind_C name needs to be
// precon_wgmres
}

extern "C" {

void printNewtonKrylovStats(
    Teuchos::ParameterList &nlParams);

void printPreconStats(Teuchos::ParameterList &lsParams_p);

void noxinit(int *nelems, double *statevector,
             int *mpi_comm_ignored, void *blackbox_res,
             void *blackbox_prec, void *jac_data) {
  void (*residualFunction)(double *, double *, int,
                           void *) = calc_f;
  // pal the 'old' precon function is obsolete

  // void (*precFunction)(double *, double *, int, double*,
  // void *, void *) = precon;
  ...
  // void (*precFunctionblock11)(double *, int, double*,
  // void *) = sw_picard_block_11alt;
  void (*precFunctionblock11)(double *, int, double *,
                              void *) = sw_picard_block_11;
  // void (*precFunctionblock11)(double *, int, double*,
  // void *) = sw_picard_block_FDiagInv;
  // void (*precFunctionblock11)(double *, int, double*,
  // void *) = sw_picard_block_FDiag;
  // void (*precFunctionblock11)(double *, int, double*,
  // void *) = sw_picard_block_ID11;
  void (*precFunctionblock12)(double *, int, double *,
                              void *) = sw_picard_DFinvBt;
  // void (*precFunctionblock12)(double *, int, double*,
  // void *) = sw_picard_block_12;
  void (*precFunctionblock21)(double *, int, double *,
                              void *) = sw_picard_block_21;
  void (*precFunctionblock22)(double *, int, double *,
                              void *) = sw_picard_schur;
  // void (*precFunctionblock22)(double *, int, double*,
  // void *) = sw_picard_block_ID22;
  // void (*precFunctionblock22)(double *, int, double*,
  // void *) = sw_picard_block_12;//Testing only
  // void (*precFunctionblock22)(double *, int, double*,
  // void *) = sw_picard_block_22;
  ...
  // void *) = sw_picard_block_21;
  // void (*precFunctionblock22)(double *, int, double*,
  // void *) = sw_picard_alphaschur;

  // void (*auxprecFunctionblock11)(double *, int, double*,
  // void *) = sw_picard_block_ID11;
  void (*auxprecFunctionblock11)(
      double *, int, double *, void *) = sw_picard_block_11;
  // void (*auxprecFunctionblock11)(double *, int, double*,
  // void *) = sw_picard_block_FDiagInv;
  // void (*auxprecFunctionblock11)(double *, int, double*,
  // void *) = sw_picard_block_FDiag;
  // void (*auxprecFunctionblock12)(double *, int, double*,
  // void *) = sw_picard_FDFinvBt;
  void (*auxprecFunctionblock12)(double *, int, double *,
                                 void *) =
      sw_picard_FDFinvBt2;
  ...
  // void (*auxprecFunctionblock12)(double *, int, double*,
  // void *) = sw_picard_DFinvBt;
  void (*auxprecFunctionblock21)(
      double *, int, double *, void *) = sw_picard_block_21;
  // void (*auxprecFunctionblock22)(double *, int, double*,
  // void *) = sw_picard_block_22;
  void (*auxprecFunctionblock22)(double *, int, double *,
                                 void *) =
      sw_picard_alphaschur;
  // void (*auxprecFunctionblock22)(double *, int, double*,
  // void *) = sw_picard_schur;
  // void (*auxprecFunctionblock22)(double *, int, double*,
  // void *) = sw_picard_block_ID22;
  ...
  try {
    ...
    // ************************************
    //
    // Create and initialize the continuation/bifurcation
    // parameter vector
    ...
    Teuchos::ParameterList &locaParamsList =
        paramList->sublist("LOCA");
    Teuchos::ParameterList &locaStepperList =
        locaParamsList.sublist("Stepper");
    locaStepperList.set("Continuation Parameter",
                        "ContinuationParam");
    //
    Teuchos::ParameterList &nlParams =
        paramList->sublist("NOX");
    Teuchos::ParameterList &nlDir =
        nlParams.sublist("Direction");
    Teuchos::ParameterList &nlNewton =
        nlDir.sublist("Newton");
    //
    Teuchos::ParameterList &stratParams =
        nlNewton.sublist("Stratimikos");
    Teuchos::ParameterList &nlPrintParams =
        nlParams.sublist("Printing");
    nlPrintParams.set("MyPID", MyPID);
    if(nlPrintParams.isParameter("Output Information"))
      //      if (!nlPrintParams.isParameter("Output
      //      Information"))
      //             nlPrintParams.set("Output
      //             Information",0
      nlPrintParams.set(
          "Output Information",
...
/* Create the interface between the test problem and the
   nonlinear solver
   This is created by the user using inheritance of the
   abstract base class:
*/
...
#if 1
    interface = Teuchos::rcp(new Problem_Interface(
        N, statevector, pVector, *Comm, blackbox_res,
        blackbox_prec, residualFunction, precFunction,
        precUpdateFunction));
#endif
...
//                                                                                                                                                                                                                                                                                                                                                                   /* interface for comparing two different formulations of SIMPLE  */
    ...
    //     if (printproc) cout<<"setup NOX interface
    //     components"<<endl<<flush;
    ...
    // Interface is inherited from these 2 classes as well
    // for user prec
...
/*Finite Difference Jacobian*/
    ...
    Teuchos::RCP<NOX::Epetra::MatrixFree> FD =
        Teuchos::rcp(new NOX::Epetra::MatrixFree(
            nlPrintParams, interface, soln));
    // Teuchos::rcp(new
    // NOX::Epetra::MatrixFree(nlPrintParams, interface,
    // soln));
...
// Analytic Jacobian
    ...
    // Create the linear systems & give Strat the matrix
    // free Jacobian, precon and precon operator
    Teuchos::RCP<NOX::Epetra::LinearSystemStratimikos>
        linsys = Teuchos::rcp(
            new NOX::Epetra::LinearSystemStratimikos(
                nlPrintParams, nlNewton,
                // Teuchos::rcp(new
                // NOX::Epetra::LinearSystemStratimikos(nlPrintParams,
                // stratParams,
                iJac, FD, iPrec, precOperator, soln, true));

    // Create the loca vector
    initialGuess = Teuchos::RCP<NOX::Epetra::Vector>(
        new NOX::Epetra::Vector(soln));

    // LOCA specific, but here grp is one branch of eventual
    // suite of parameter spaces
    ...
    Teuchos::RCP<LOCA::Epetra::Group> grp =
        Teuchos::rcp(new LOCA::Epetra::Group(
            globalData, nlPrintParams, iReq, *initialGuess,
            linsys, pVector));
    ...
    const double wrms_tol =
        nlParams.get("Convergence Tolerance", 1.0e-10);
    wrms =
        Teuchos::rcp(new NOX::StatusTest::NormF(wrms_tol));
    Teuchos::RCP<NOX::StatusTest::MaxIters> maxiters =
        Teuchos::rcp(new NOX::StatusTest::MaxIters(10));
    Teuchos::RCP<NOX::StatusTest::Combo> combo =
        Teuchos::rcp(new NOX::StatusTest::Combo(
            NOX::StatusTest::Combo::OR));
    combo->addStatusTest(wrms);
    combo->addStatusTest(maxiters);

    Teuchos::RCP<Teuchos::ParameterList> nlParamsRCP =
        Teuchos::rcp(&nlParams, false);
    solver =
        NOX::Solver::buildSolver(grp, combo, nlParamsRCP);
...
void noxsolve(int *nelems, double *statevector,
  ...
  try {
    TEUCHOS_TEST_FOR_EXCEPTION(
        is_null(solver), logic_error,
        "Exception: noxsolve called with solver=null: "
            << "either did call noxinit 1st, or called "
               "noxfinish already");

    // reset solver with new initial Guess
    Epetra_Vector &initialGuessEV =
        initialGuess->getEpetraVector();
    for(int i = 0; i < *nelems; i++)
      initialGuessEV[i] = statevector[i];
    ...
    if(globalData->locaUtils->isPrintType(
           NOX::Utils::StepperParameters)) {
    ...
    const Epetra_Vector &finalSolution =
        (dynamic_cast<const NOX::Epetra::Vector &>(
             solver->getSolutionGroup().getX()))
            .getEpetraVector();
    for(int i = 0; i < *nelems; i++)
      statevector[i] = finalSolution[i];
  } catch(std::exception &e) {
    cout << e.what() << endl;
  } catch(const char *s) {
    cout << s << endl;
...
/*
  void precon_solve(int* nelems, double* statevector,
  double* rhs,
      void* blackbox_res, void* blackbox_prec)
  {

    try {
      TEUCHOS_TEST_FOR_EXCEPTION(is_null(solver_p),
  logic_error,
          "Exception: precon_solve called with solver=null:
  "
          << "either did call init_prec 1st, or called
  finish_prec already");

      // reset solver with new initial Guess
      Epetra_Vector& initialGuessEVp =
  initialGuess_p->getEpetraVector();
      for (int i=0; i<*nelems; i++) initialGuessEVp[i] =
  statevector[i];
      // maybe a redundancy - check
      solver_p->reset(*initialGuess_p);

      // convert rhs from double * to epetra vector(just
  like for statevector)
      Epetra_Vector& initialGuessEVr =
  initialGuess_r->getEpetraVector();
      for (int i=0; i<*nelems; i++) initialGuessEVr[i] =
  rhs[i];

      // convert solution to appplyJI from double * to
  epetra vector
      Epetra_Vector& initialGuessEVv =
  initialGuess_v->getEpetraVector();
      for (int i=0; i<*nelems; i++) initialGuessEVv[i] =
  rhs[i];

      // reset interface with new blackbox
      interface_p->resetBlackbox(blackbox_res,
  blackbox_prec);

      //    Teuchos::ParameterList& lsParams_p =
      //
  const_cast<Teuchos::ParameterList&>(solver_p->getList());
      //    lsParams_p.sublist("Linear
  Solver").sublist("Output").
      //                        set("Total Number of Linear
  Iterations", 1);
      ...
      nlParams_p.sublist("Direction").sublist("Newton").
                 ...
                 set("Total Number of Linear Iterations",
  0);

      Teuchos::ParameterList& lsParams_p =
        nlParams_p.sublist("Direction").sublist("Newton").sublist("Linear
  Solver");

      // call Apply Jacobian inverse method with z as the
  argument
      grp_p->setX(*initialGuess_p);
      NOX::Abstract::Group::ReturnType stat1 =
  grp_p->computeJacobian();
...
void printNewtonKrylovStats(
  ...
  int NewtonIters = nlParams.sublist("Output").get(
  ...
  stepNum++;
  globalData->locaUtils->out()
      << "Convergence Stats: for step  #" << stepNum
      << " : Newton, Krylov, Kr/Ne: " << NewtonIters << "  "
      << KrylovIters << "  "
      << (double)KrylovIters /
             (double)(NewtonIters + 1.0e-14)
      << endl;
  if(stepNum > 1) {
    globalData->locaUtils->out()
        << "Convergence Stats: running total: Nn, Kr, "
           "Kr/Ne, Kr/Step: "
        << totalNewtonIters << "  " << totalKrylovIters
        << "  "
        << (double)totalKrylovIters /
               (double)totalNewtonIters
        << "  "
...
/*
  void printPreconStats(Teuchos::ParameterList& lsParams_p)
  {
    ...
    static int stepNum=0;
    int KrylovIters =
  lsParams_p.sublist("Direction").sublist("Newton").
                                 sublist("Linear
  Solver").sublist("Output").
                                 get("Total Number of Linear
  Iterations", -1000);
    totalKrylovIters += KrylovIters;
    stepNum++;
    globalData_p->locaUtils->out() << "Precon Convergence
  Stats: for step  #" << stepNum
                                   << " : Krylov " <<
  KrylovIters << endl;
    if (stepNum > 1) {
      globalData_p->locaUtils->out() << "Precon Convergence
  Stats: running Krylov total: "
                                     << totalKrylovIters <<
  "  " << endl;
    }
  }

...
// void printPreconStats(Teuchos::ParameterList&
// lsParams_p);
...
void noxinit(int *nelems, double *statevector,
  ...
  try {
    ...
    interface = Teuchos::rcp(new Problem_Interface(
        nelems, statevector, pVector, Comm, blackbox_res,
        blackbox_prec, residualFunction, precFunction));
    //  interface = Teuchos::rcp(new
    //  Problem_Interface(nelems, statevector, pVector,
    //  Comm,
    //                                       blackbox_res,
    //                                       blackbox_prec,
    //                                       residualFunction));
    ...
    //  linsys =
    //    Teuchos::rcp(new
    //    NOX::Epetra::LinearSystemAztecOO(nlPrintParams,
    //    lsParams,
    //  				         iJac, FD, iPrec, precOperator,
    //  soln));

    // STRAT3 giving Strat the matrix free Jacobian, precon
    // and precon operator
    Teuchos::RCP<NOX::Epetra::LinearSystemStratimikos>
        linsysstrat = Teuchos::rcp(
            new NOX::Epetra::LinearSystemStratimikos(
                nlPrintParams, stratParams, iJac, FD, iPrec,
                precOperator, soln, true));
    ...
    Teuchos::RCP<NOX::StatusTest::NormF> wrms =
        Teuchos::rcp(new NOX::StatusTest::NormF(
            nlParams.get("Convergence Tolerance", 1.0e-8)));

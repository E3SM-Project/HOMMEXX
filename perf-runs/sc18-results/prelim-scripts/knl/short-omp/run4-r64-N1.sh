...
tt=1 #threads
NMPI=$((PER_NODE*NN))
comm="srun -N ${NN} -n ${NMPI} -c 4 --cpu_bind=cores "
...
mv HommeTime_stats time-homme-ne8-N${NN}-r${PER_NODE}-t${tt}-iter${i}-${dd}

$comm $execxx < $nlxx &> tmpxx.txt
tail -n 5 tmp.txt
mv HommeTime_stats time-xx-ne8-N${NN}-r${PER_NODE}-t${tt}-iter${i}-${dd}
...
#repetition
n1=time-xx-ne8-N${NN}-r${PER_NODE}-t${tt}-iter${i}-${dd}
n2=time-homme-ne8-N${NN}-r${PER_NODE}-t${tt}-iter${i}-${dd}

...
void caar_compute_pressure_f90(const int &nets, const int &nete,
                               const int &nelemd, const int &n0,
                               const Real &hyai_ps0, RCPtr &p_ptr,
                               RCPtr &dp_ptr);

void caar_compute_vort_and_div_c(
    const int &nets, const int &nete, const int &nelemd, const int &n0,
...
void caar_compute_vort_and_div_f90(
    const int &nets, const int &nete, const int &nelemd, const int &n0,
    const Real &eta_ave_w, CRCPtr &dvv_ptr, CRCPtr &D_ptr, CRCPtr &Dinv_ptr,
    CRCPtr &metdet_ptr, CRCPtr &rmetdet_ptr, CRCPtr &p_ptr, CRCPtr &dp_ptr,
    RCPtr &grad_p_ptr, RCPtr &vgrad_p_ptr, CRCPtr &v_ptr, RCPtr &vn0_ptr,
...
void caar_compute_t_v_c(const int &nets, const int &nete, const int &nelemd,
                        ...
                        RCPtr &T_v_ptr, RCPtr &kappa_star_ptr, CRCPtr &dp_ptr,
                        CRCPtr &Temp_ptr, CRCPtr &Qdp_ptr);
...
void flip_array_np_np_2_2_nelems(RCPtr &arr_f, RCPtr &arr_c,
                                 const int num_elems) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    arr_c[ie * dim1234 + idim * dim123 + jdim * dim12 + igp * NP + jgp] =
        arr_f[i];
  }
}

void flip_array_np_np_nlev_nelems(RCPtr &arr_f, RCPtr &arr_c,
                                  const int num_elems) {
  // Note: each np x np block should not be flipped, that is,
...
void flip_array_np_np_2_nlev_nelems(RCPtr &arr_f, RCPtr &arr_c,
                                    const int num_elems) {
  // Note: each np x np block should not be flipped, that is,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    arr_c[ie * dim1234 + ilev * dim123 + idim * dim12 + igp * NP + jgp] =
        arr_f[i];
  }
}

void flip_array_np_np_2_nlev_timelevels_nelems(RCPtr &arr_f, RCPtr &arr_c,
                                               const int num_elems) {
  // Note: each np x np block should not be flipped, that is,
  //       (igp,jgp,idim,ilev,itlev,ielem)_f ->
  // (ielem,itlev,ilev,idim,igp,jgp)_c
  ...
  for (int i = 0; i < size; ++i) {
    ...
    arr_c[ie * dim12345 + itl * dim1234 + ilev * dim123 + dim * dim12 +
          igp * NP + jgp] = arr_f[i];
  }
}

void flip_array_np_np_nlev_timelevels_nelems(RCPtr &arr_f, RCPtr &arr_c,
                                             const int num_elems) {
  // Note: each np x np block should not be flipped, that is,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    arr_c[ie * dim1234 + itl * dim123 + ilev * dim12 + igp * NP + jgp] =
        arr_f[i];
  }
}
...
void flip_array_Qdp(RCPtr &arr_f, RCPtr &arr_c, const int num_elems) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    arr_c[ie * dim12345 + tlq * dim1234 + iq * dim123 + ilev * dim12 +
          igp * NP + jgp] = arr_f[i];
  }
}
...
TEST_CASE("compute_and_apply_rhs / 1", "compute_pressure") {
  ...
  SECTION("random_test") {
    ...
    for (int itest = 0; itest < num_rand_test; ++itest) {
      ...
      const int nete =
          udi_type(std::min(nets + 1, num_elems), num_elems)(engine);
      const int n0 = udi_type(1, 3)(engine);
      const Real hyai_ps0 = dreal(engine);

      // To avoid false checks on elements not actually processed in this random
      // test
      ...
      // Compute
      caar_compute_pressure_c(nets, nete, num_elems, n0, hyai_ps0, p_cxx,
                              dp_cxx);
      caar_compute_pressure_f90(nets, nete, num_elems, n0, hyai_ps0, p_f90,
                                dp_f90);
...
TEST_CASE("compute_and_apply_rhs / 2", "compute_vort_and_div") {
  ...
  SECTION("random_test") {
    ...
    for (int itest = 0; itest < num_rand_test; ++itest) {
      ...
      // Compute
      caar_compute_vort_and_div_c(nets, nete, num_elems, n0, eta_ave_w, dvv_cxx,
                                  D_cxx, Dinv_cxx, metdet_cxx, rmetdet_cxx,
                                  p_cxx, dp_cxx, grad_p_cxx, vgrad_p_cxx, v_cxx,
                                  vn0_cxx, vdp_cxx, div_vdp_cxx, vort_cxx);
      caar_compute_vort_and_div_f90(
          nets, nete, num_elems, n0, eta_ave_w, dvv_f90, D_f90, Dinv_f90,
...
TEST_CASE("compute_and_apply_rhs / 3", "compute_T_v") {
  ...
  constexpr int size4d = num_elems * NUM_TIME_LEVELS * NUM_LEV * NP * NP;
  constexpr int sizeQ =
      num_elems * Q_NUM_TIME_LEVELS * QSIZE_D * NUM_LEV * NP * NP;
  ...
  SECTION("random_test") {
    ...
    for (int itest = 0; itest < num_rand_test; ++itest) {
      ...
      const int qn0 =
          bernoulli(engine) == 0 ? -1 : udi_type(1, Q_NUM_TIME_LEVELS)(engine);
      ...
      // Compute
      caar_compute_t_v_c(nets, nete, num_elems, n0, qn0, use_cpstar, T_v_cxx,
                         kappa_star_cxx, dp_cxx, T_cxx, Qdp_cxx);
      caar_compute_t_v_f90(nets, nete, num_elems, n0, qn0, use_cpstar, T_v_f90,
                           kappa_star_f90, dp_f90, T_f90, Qdp_f90);

      // Check the answers
      flip_array_np_np_nlev_nelems(T_v_f90, T_v_f90_cxx, num_elems);
      flip_array_np_np_nlev_nelems(kappa_star_f90, kappa_star_f90_cxx,
                                   num_elems);

      for (int i = 0; i < size3d; ++i) {
        REQUIRE(compare_answers(T_v_f90_cxx[i], T_v_cxx[i]) == 0.0);
        REQUIRE(compare_answers(kappa_star_f90_cxx[i], kappa_star_cxx[i]) ==
                0.0);
      }
    }
  }

...
struct CaarFunctorImpl {
  ...
  KOKKOS_INLINE_FUNCTION void compute_energy_grad(KernelVariables &kv) const {
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                // pre-fill energy_grad with the pressure(_grad)-temperature
                // part
                m_elements.buffers.energy_grad(kv.team_idx, 0, igp, jgp, ilev) =
                    PhysicalConstants::Rgas *
                    (m_elements.buffers.temperature_virt(kv.team_idx, igp, jgp,
                                                         ilev) /
                     m_elements.buffers.pressure(kv.team_idx, igp, jgp, ilev)) *
                    m_elements.buffers.pressure_grad(kv.team_idx, 0, igp, jgp,
                                                     ilev);
                ...
                m_elements.buffers.energy_grad(kv.team_idx, 1, igp, jgp, ilev) =
                    ...
                    (m_elements.buffers.temperature_virt(kv.team_idx, igp, jgp,
                                                         ilev) /
                     m_elements.buffers.pressure(kv.team_idx, igp, jgp, ilev)) *
                    m_elements.buffers.pressure_grad(kv.team_idx, 1, igp, jgp,
                                                     ilev);
    ...
    m_sphere_ops.gradient_sphere_update(
        kv, Homme::subview(m_elements.buffers.ephi, kv.team_idx),
  ...
  void compute_velocity_np1(KernelVariables &kv) const {
    ...
    m_sphere_ops.vorticity_sphere(
        kv, Homme::subview(m_elements.m_v, kv.ie, m_data.n0),
        ...
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                m_elements.buffers.energy_grad(kv.team_idx, 0, igp, jgp,
                                               ilev) *= -1;

                m_elements.buffers.energy_grad(kv.team_idx, 0, igp, jgp,
                                               ilev) +=
                    (rsplit_gt0 ? 0
                                : -m_elements.buffers.v_vadv_buf(
                                      kv.team_idx, 0, igp, jgp, ilev)) +
                    m_elements.m_v(kv.ie, m_data.n0, 1, igp, jgp, ilev) *
                        m_elements.buffers.vorticity(kv.team_idx, igp, jgp,
                                                     ilev);

                m_elements.buffers.energy_grad(kv.team_idx, 1, igp, jgp,
                                               ilev) *= -1;
  ...
  void compute_eta_dot_dpdn_vertadv_euler(KernelVariables &kv) const {
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int idx) {
          ...
          // one more entry for sdot, separately, cause eta_dot_ is not of size
          // LEV+1
          {
          ...
          for (int k = 1; k < NUM_PHYSICAL_LEV; ++k) {
            ...
            m_elements.buffers.eta_dot_dpdn_buf(kv.team_idx, igp, jgp,
                                                ilev)[ivec] =
                m_hvcoord.hybrid_bi(k) *
                    m_elements.buffers.sdot_sum(kv.team_idx, igp, jgp) -
                m_elements.buffers.eta_dot_dpdn_buf(kv.team_idx, igp, jgp,
                                                    ilev)[ivec];
          }  // k loop
          m_elements.buffers.eta_dot_dpdn_buf(kv.team_idx, igp, jgp, 0)[0] =
              0.0;
        });  // NP*NP loop
    kv.team_barrier();
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int idx) {
          ...
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
                               [&](const int &ilev) {
                                 m_elements.buffers.temperature_virt(
                                     kv.team_idx, igp, jgp, ilev) =
                                     m_elements.m_t(kv.ie, m_data.n0, igp, jgp,
                                                    ilev);
                               });
        });
    kv.team_barrier();
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                Scalar Qt =
                    m_tracers.qdp(kv.ie, m_data.n0_qdp, 0, igp, jgp, ilev) /
  ...
  void compute_div_vdp(KernelVariables &kv) const {
    ...
    m_sphere_ops.divergence_sphere(
        kv, Homme::subview(m_elements.buffers.vdp, kv.team_idx),
          ...
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
                               [&](const int &ilev) {
                                 m_elements.m_omega_p(kv.ie, igp, jgp, ilev) +=
                                     m_data.eta_ave_w *
                                     m_elements.buffers.omega_p(kv.team_idx,
                                                                igp, jgp, ilev);
                               });
        });
    kv.team_barrier();
  ...
  void compute_temperature_np1(KernelVariables &kv) const {
    m_sphere_ops.gradient_sphere(
        kv, Homme::subview(m_elements.m_t, kv.ie, m_data.n0),
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                const Scalar vgrad_t =
                    m_elements.m_v(kv.ie, m_data.n0, 0, igp, jgp, ilev) *
                        m_elements.buffers.temperature_grad(kv.team_idx, 0, igp,
                                                            jgp, ilev) +
                    m_elements.m_v(kv.ie, m_data.n0, 1, igp, jgp, ilev) *
                        m_elements.buffers.temperature_grad(kv.team_idx, 1, igp,
                                                            jgp, ilev);

                const Scalar ttens =
                    (rsplit_gt0 ? 0
                                : -m_elements.buffers.t_vadv_buf(
                                      kv.team_idx, igp, jgp, ilev)) -
                    vgrad_t +
                    PhysicalConstants::kappa *
                        m_elements.buffers.temperature_virt(kv.team_idx, igp,
                                                            jgp, ilev) *
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int idx) {
          ...
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
              [&](const int &ilev) {
                Scalar tmp = m_elements.buffers.eta_dot_dpdn_buf(
                    kv.team_idx, igp, jgp, ilev);
                tmp.shift_left(1);
                tmp[VECTOR_SIZE - 1] =
                    (ilev + 1 < NUM_LEV)
                        ? m_elements.buffers.eta_dot_dpdn_buf(kv.team_idx, igp,
                                                              jgp, ilev + 1)[0]
                        : 0;
                // Add div_vdp before subtracting the previous value to
                // eta_dot_dpdn This will hopefully reduce numeric error
                tmp += m_elements.buffers.div_vdp(kv.team_idx, igp, jgp, ilev);
                tmp -= m_elements.buffers.eta_dot_dpdn_buf(kv.team_idx, igp,
                                                           jgp, ilev);
  ...
  void preq_vertadv(KernelVariables &kv) const {
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int idx) {
          ...
          // lets do this 1/dp thing to make it bfb with F and follow F for
          // extra
          // (), not clear why
          Real facp =
              (0.5 * 1 /
          ...
          m_elements.buffers.t_vadv_buf(kv.team_idx, igp, jgp, ilev)[ivec] =
              facp *
              (m_elements.m_t(kv.ie, m_data.n0, igp, jgp, ilevp1)[ivecp1] -
          ...
          for (int k = 1; k < NUM_PHYSICAL_LEV - 1; ++k) {
            ...
            m_elements.buffers.v_vadv_buf(kv.team_idx, 0, igp, jgp,
                                          ilev)[ivec] =
                facp * (m_elements.m_v(kv.ie, m_data.n0, 0, igp, jgp,
                                       ilevp1)[ivecp1] -
                        m_elements.m_v(kv.ie, m_data.n0, 0, igp, jgp,
                                       ilev)[ivec]) +
                facm *
                    (m_elements.m_v(kv.ie, m_data.n0, 0, igp, jgp, ilev)[ivec] -
                     m_elements.m_v(kv.ie, m_data.n0, 0, igp, jgp,
            ...
            m_elements.buffers.v_vadv_buf(kv.team_idx, 1, igp, jgp,
                                          ilev)[ivec] =
                facp * (m_elements.m_v(kv.ie, m_data.n0, 1, igp, jgp,
                                       ilevp1)[ivecp1] -
                        m_elements.m_v(kv.ie, m_data.n0, 1, igp, jgp,
                                       ilev)[ivec]) +
                facm *
                    (m_elements.m_v(kv.ie, m_data.n0, 1, igp, jgp, ilev)[ivec] -
                     m_elements.m_v(kv.ie, m_data.n0, 1, igp, jgp,
          ...
          facm = (0.5 * (1 / m_elements.m_dp3d(kv.ie, m_data.n0, igp, jgp,
                                               ilev)[ivec])) *
                 m_elements.buffers.eta_dot_dpdn_buf(kv.team_idx, igp, jgp,
                                                     ilev)[ivec];
 ...
 private:
  ...
  KOKKOS_INLINE_FUNCTION
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int loop_idx) {
          ...
          ExecViewUnmanaged<Real[NUM_PHYSICAL_LEV]> p(
              &m_elements.buffers.pressure(kv.team_idx, igp, jgp, 0)[0]);
          ExecViewUnmanaged<const Real[NUM_PHYSICAL_LEV]> dp(
              &m_elements.m_dp3d(kv.team_idx, m_data.n0, igp, jgp, 0)[0]);

          const Real p0 = m_hvcoord.hybrid_ai0 * m_hvcoord.ps0 + 0.5 * dp(0);
          Kokkos::single(Kokkos::PerThread(kv.team), [&]() { p(0) = p0; });
          // Compute cumsum of (dp(k-1) + dp(k)) in [1,NUM_PHYSICAL_LEV] and
          // update p(k) For level=1 (which is k=0), first add p0=hyai0*ps0 +
          // 0.5*dp(0).
          Dispatch<ExecSpaceType>::parallel_scan(
              kv.team, NUM_PHYSICAL_LEV - 1,
              [&](const int k, Real &accumulator, const bool last) {

                // Note: the actual level must range in [1,NUM_PHYSICAL_LEV],
                // while k ranges in [0, NUM_PHYSICAL_LEV-1]. If k=0, add the p0
                // part: hyai0*ps0 + 0.5*dp(0)
  ...
  KOKKOS_INLINE_FUNCTION
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int loop_idx) {
          ...
          // Note: we add VECTOR_SIZE-1 rather than subtracting 1 since
          // (0-1)%N=-1 while (0+N-1)%N=N-1.
          ...
          for (int ilev = NUM_LEV - 1; ilev >= 0; --ilev) {
            ...
            const auto &t_v = m_elements.buffers.temperature_virt(
                kv.team_idx, igp, jgp, ilev);
            const auto &dp3d =
                m_elements.m_dp3d(kv.ie, m_data.n0, igp, jgp, ilev);
            const auto &p =
  ...
  static KOKKOS_INLINE_FUNCTION void assert_vector_size_1() {
    ...
    if (VECTOR_SIZE != 1)
      Kokkos::abort(
          "This impl is for GPU, for which VECTOR_SIZE is 1. It will "
  ...
  KOKKOS_INLINE_FUNCTION
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int loop_idx) {
          ...
          const auto rgas_tv_dp_over_p = Homme::subview(
              m_elements.buffers.vstar, kv.team_idx, 0, igp, jgp);
          const auto integration = Homme::subview(
              m_elements.buffers.divergence_temp, kv.team_idx, igp, jgp);
          ...
          Dispatch<ExecSpaceType>::parallel_scan(
              kv.team, NUM_PHYSICAL_LEV - 1,
              [&](const int k, Real &accumulator, const bool last) {
                // level must range in [NUM_PHYSICAL_LEV-1,1], while k ranges in
                // [0, NUM_PHYSICAL_LEV-2].
          ...
          // Update phi with a parallel_for: phi(k)= phis + 2.0 *
          // integration(k+1) + rgas_tv_dp_over_p(k);
          const Real phis = m_elements.m_phis(kv.ie, igp, jgp);
          const auto phi = Homme::subview(m_elements.m_phi, kv.ie, igp, jgp);
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
                               [&](const int level) {
                                 phi(level) = phis + 2.0 * integration(level) +
                                              rgas_tv_dp_over_p(level);
                               });
        });
    kv.team_barrier();
  }
  ...
  KOKKOS_INLINE_FUNCTION
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int loop_idx) {
          ...
          ExecViewUnmanaged<Real[NUM_PHYSICAL_LEV]> omega_p(
              &m_elements.buffers.omega_p(kv.team_idx, igp, jgp, 0)[0]);
          ExecViewUnmanaged<Real[NUM_PHYSICAL_LEV]> div_vdp(
              &m_elements.buffers.div_vdp(kv.team_idx, igp, jgp, 0)[0]);
          Dispatch<ExecSpaceType>::parallel_scan(
          ...
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&](const int ilev) {
                const Scalar vgrad_p =
                    m_elements.m_v(kv.ie, m_data.n0, 0, igp, jgp, ilev) *
                        m_elements.buffers.pressure_grad(kv.team_idx, 0, igp,
                                                         jgp, ilev) +
                    m_elements.m_v(kv.ie, m_data.n0, 1, igp, jgp, ilev) *
                        m_elements.buffers.pressure_grad(kv.team_idx, 1, igp,
                                                         jgp, ilev);
                ...
                m_elements.buffers.omega_p(kv.team_idx, igp, jgp, ilev) =
                    (vgrad_p -
                     (m_elements.buffers.omega_p(kv.team_idx, igp, jgp, ilev) +
                      0.5 * m_elements.buffers.div_vdp(kv.team_idx, igp, jgp,
                                                       ilev))) /
                    p;
              });
        });

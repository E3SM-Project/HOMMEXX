...
void Elements::init(const int num_elems, const bool consthv) {
  ...
  if (!consthv) {
    m_tensorvisc =
        ExecViewManaged<Real * [2][2][NP][NP]>("TENSORVISC", m_num_elems);
    m_vec_sph2cart =
        ExecViewManaged<Real * [2][3][NP][NP]>("VEC_SPH2CART", m_num_elems);
  }
  ...
  m_dinv = ExecViewManaged<Real * [2][2][NP][NP]>("DInv - inverse of matrix D",
                                                  m_num_elems);
  ...
  m_ps_v =
      ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP]>("PS_V", m_num_elems);
  ...
  m_derived_dp =
      ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>("derived_dp", m_num_elems);
  m_derived_divdp =
      ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>("derived_divdp", m_num_elems);
...
void Elements::init_2d(CF90Ptr &D, CF90Ptr &Dinv, CF90Ptr &fcor, CF90Ptr &mp,
                       CF90Ptr &spheremp, CF90Ptr &rspheremp, CF90Ptr &metdet,
                       CF90Ptr &metinv, CF90Ptr &phis, CF90Ptr &tensorvisc,
  ...
  HostViewUnmanaged<const Real * [2][2][NP][NP]> h_metinv_f90(metinv,
                                                              m_num_elems);
      ...
      for (int jdim = 0; jdim < 2; ++jdim) {
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp, ++k_tensors) {
            ...
            h_metinv(ie, idim, jdim, igp, jgp) =
                h_metinv_f90(ie, idim, jdim, igp, jgp);
          }
        }
      }
    }
    ...
    for (int ie = 0; ie < m_num_elems; ++ie) {
      for (int idim = 0; idim < 2; ++idim) {
        for (int jdim = 0; jdim < 3; ++jdim) {
          for (int igp = 0; igp < NP; ++igp) {
            for (int jgp = 0; jgp < NP; ++jgp, ++k_tensors) {
              h_vec_sph2cart(ie, idim, jdim, igp, jgp) =
                  vec_sph2cart[k_tensors];
            }
          }
        }
      }  // init 2d quantities
...
void Elements::random_init(const int num_elems, const Real max_pressure) {
  ...
  genRandArray(m_ps_v, engine,
               std::uniform_real_distribution<Real>(100 * hvcoord.ps0,
                                                    1000 * hvcoord.ps0));
  ...
  const auto make_pressure_partition =
      [=](HostViewUnmanaged<Scalar[NUM_LEV]> pt_pressure) {
        ...
        // This is okay, because we only need to approximately sum to
        // max_pressure
  ...
  genRandArray(
      m_eta_dot_dpdn, engine,
      std::uniform_real_distribution<Real>(0.01 * dp3d_min, 0.1 * dp3d_min));
...
void Elements::pull_from_f90_pointers(CF90Ptr &state_v, CF90Ptr &state_t,
...
void Elements::pull_3d(CF90Ptr &derived_phi, CF90Ptr &derived_omega_p,
                       CF90Ptr &derived_v) {
  HostViewUnmanaged<const Real * [NUM_PHYSICAL_LEV][NP][NP]> derived_phi_f90(
      derived_phi, m_num_elems);
  HostViewUnmanaged<const Real * [NUM_PHYSICAL_LEV][NP][NP]>
      derived_omega_p_f90(derived_omega_p, m_num_elems);
  HostViewUnmanaged<const Real * [NUM_PHYSICAL_LEV][2][NP][NP]> derived_v_f90(
      derived_v, m_num_elems);
...
void Elements::pull_4d(CF90Ptr &state_v, CF90Ptr &state_t,
                       CF90Ptr &state_dp3d) {
  HostViewUnmanaged<const Real * [NUM_TIME_LEVELS][NUM_PHYSICAL_LEV][NP][NP]>
      state_t_f90(state_t, m_num_elems);
  HostViewUnmanaged<const Real * [NUM_TIME_LEVELS][NUM_PHYSICAL_LEV][NP][NP]>
      state_dp3d_f90(state_dp3d, m_num_elems);
  HostViewUnmanaged<const Real * [NUM_TIME_LEVELS][NUM_PHYSICAL_LEV][2][NP][NP]>
      state_v_f90(state_v, m_num_elems);
...
void Elements::pull_eta_dot(CF90Ptr &derived_eta_dot_dpdn) {
  HostViewUnmanaged<const Real * [NUM_INTERFACE_LEV][NP][NP]> eta_dot_dpdn_f90(
      derived_eta_dot_dpdn, m_num_elems);
...
void Elements::push_3d(F90Ptr &derived_phi, F90Ptr &derived_omega_p,
                       F90Ptr &derived_v) const {
  HostViewUnmanaged<Real * [NUM_PHYSICAL_LEV][NP][NP]> derived_phi_f90(
      derived_phi, m_num_elems);
  HostViewUnmanaged<Real * [NUM_PHYSICAL_LEV][NP][NP]> derived_omega_p_f90(
      derived_omega_p, m_num_elems);
  HostViewUnmanaged<Real * [NUM_PHYSICAL_LEV][2][NP][NP]> derived_v_f90(
      derived_v, m_num_elems);
...
void Elements::push_4d(F90Ptr &state_v, F90Ptr &state_t,
  ...
  HostViewUnmanaged<Real * [NUM_TIME_LEVELS][NUM_PHYSICAL_LEV][NP][NP]>
      state_dp3d_f90(state_dp3d, m_num_elems);
  HostViewUnmanaged<Real * [NUM_TIME_LEVELS][NUM_PHYSICAL_LEV][2][NP][NP]>
      state_v_f90(state_v, m_num_elems);
...
void Elements::push_eta_dot(F90Ptr &derived_eta_dot_dpdn) const {
  HostViewUnmanaged<Real * [NUM_INTERFACE_LEV][NP][NP]> eta_dot_dpdn_f90(
      derived_eta_dot_dpdn, m_num_elems);
...
void Elements::dinv(Real *dinv_ptr, int ie) const {
  ExecViewUnmanaged<Real[2][2][NP][NP]> dinv_device =
      Homme::subview(m_dinv, ie);
  decltype(dinv_device)::HostMirror dinv_host =
      Kokkos::create_mirror_view(dinv_device);
...
void Elements::BufferViews::init(const int num_elems) {
  ...
  ttens = ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>(
      "Temporary for temperature", num_elems);
  dptens = ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>("Temporary for dp3d",
                                                       num_elems);
  vtens = ExecViewManaged<Scalar * [2][NP][NP][NUM_LEV]>(
      "Temporary for velocity", num_elems);

  vstar = ExecViewManaged<Scalar * [2][NP][NP][NUM_LEV]>(
      "buffer for (flux v)/dp", num_elems);
  ...
  sphere_vector_buf = ExecViewManaged<Scalar * [2][NP][NP][NUM_LEV]>(
      "laplacian vector Buffer", num_elems);

  divergence_temp = ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>(
      "Divergence temporary", num_elems);
  vorticity_temp = ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>(
      "Vorticity temporary", num_elems);
  lapl_buf_1 = ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>(
      "Scalar laplacian Buffer", num_elems);
  lapl_buf_2 = ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>(
      "Scalar laplacian Buffer", num_elems);
  lapl_buf_3 = ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>(
      "Scalar laplacian Buffer", num_elems);
  v_vadv_buf = ExecViewManaged<Scalar * [2][NP][NP][NUM_LEV]>("v_vadv buffer",
                                                              num_elems);
  t_vadv_buf =
      ExecViewManaged<Scalar * [NP][NP][NUM_LEV]>("t_vadv buffer", num_elems);
  eta_dot_dpdn_buf = ExecViewManaged<Scalar * [NP][NP][NUM_LEV_P]>(
      "eta_dot_dpdpn buffer", num_elems);

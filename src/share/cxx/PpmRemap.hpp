  ...
  static void fill_cell_means_gs(
    ...
    Kokkos::parallel_for(
        Kokkos::ThreadVectorRange(kv.team, gs), [&](const int &k_0) {
          ...
          cell_means(NUM_PHYSICAL_LEV + _ppm_consts::INITIAL_PADDING - gs +
                     k_0 + 1 + 1) =
              cell_means(NUM_PHYSICAL_LEV + _ppm_consts::INITIAL_PADDING - gs +
                         1 - k_0 - 1 + 1);
        });  // end ghost cell loop
...
struct PpmVertRemap : public VertRemapAlg {
  ...
  KOKKOS_INLINE_FUNCTION
  void compute_remap_phase(
      KernelVariables &kv,
      ExecViewUnmanaged<Scalar[NP][NP][NUM_LEV]> remap_var) const {
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int &loop_idx) {
          ...
          boundaries::fill_cell_means_gs(
              kv, Homme::subview(ao, kv.team_idx, igp, jgp));

          Dispatch<ExecSpace>::parallel_scan(
              kv.team, NUM_PHYSICAL_LEV,
              [=](const int &k, Real &accumulator, const bool last) {
                // Accumulate the old mass up to old grid cell interface
                // locations to simplify integration during remapping. Also,
                // divide out the grid spacing so we're working with actual
                // tracer values and can conserve mass.
                if (last) {
        ...
        Kokkos::ThreadVectorRange(kv.team, NUM_PHYSICAL_LEV), [&](const int k) {
          const Real mass_1 =
              (k > 0) ? compute_mass(
                            ...
                            parabola_coeffs(0, k_id(k - 1)),
                            prev_mass(k_id(k - 1) + 1),
  ...
  void compute_grids(
    ...
    Kokkos::parallel_for(
        Kokkos::ThreadVectorRange(kv.team, NUM_PHYSICAL_LEV + 2),
        [&](const int j) {
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int &loop_idx) {
          ...
          ExecViewUnmanaged<Real[_ppm_consts::DPO_PHYSICAL_LEV]> point_dpo =
              Homme::subview(dpo, kv.ie, igp, jgp);
          ExecViewUnmanaged<Real[10][_ppm_consts::PPMDX_PHYSICAL_LEV]>
              point_ppmdx = Homme::subview(ppmdx, kv.ie, igp, jgp);
          compute_grids(kv, point_dpo, point_ppmdx);
        });
  }

  KOKKOS_FORCEINLINE_FUNCTION Real integrate_parabola(const Real sq_coeff,
                                                      const Real lin_coeff,
                                                      const Real const_coeff,
                                                      Real x1, Real x2) const {

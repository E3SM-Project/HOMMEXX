...
struct PpmVertRemap : public VertRemapAlg {
  ...
  void compute_remap_phase(
      KernelVariables &kv, const int remap_idx,
        ...
        Kokkos::ThreadVectorRange(kv.team, NUM_PHYSICAL_LEV), [&](const int k) {
          ...
          const Real mass_1 = [=]() {
            if (k > 0) {
              ...
              return compute_mass(parabola_coeffs(2, kk_prev_lev),
                                  parabola_coeffs(1, kk_prev_lev),
                                  parabola_coeffs(0, kk_prev_lev),
                                  prev_mass(kk_prev_lev),
...

    {
      ...
      Kokkos::parallel_for(
          ...
          [&](const int zoffset_j) {
            ...
            grids(5, j) =
                (2.0 * dx(j + 1) * dx(j + 2)) / (dx(j + 1) + dx(j + 2));

            grids(6, j) = (dx(j) + dx(j + 1)) / (2.0 * dx(j + 1) + dx(j + 2));

            grids(7, j) =
                (dx(j + 3) + dx(j + 2)) / (2.0 * dx(j + 2) + dx(j + 1));

            grids(8, j) =
                dx(j + 1) * (dx(j) + dx(j + 1)) / (2.0 * dx(j + 1) + dx(j + 2));

            grids(9, j) = dx(j + 2) * (dx(j + 2) + dx(j + 3)) /
                          (dx(j + 1) + 2.0 * dx(j + 2));
          });
    }
  }
  ...
  KOKKOS_INLINE_FUNCTION Real integrate_parabola(const Real sq_coeff,

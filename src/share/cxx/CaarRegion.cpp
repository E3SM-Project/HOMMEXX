...
void CaarRegion::init(const int num_elems) {
  ...
  m_omega_p = ExecViewManaged<Real * [NP][NP][NUM_LEV]>("Omega P", m_num_elems);
  m_pecnd = ExecViewManaged<Real * [NP][NP][NUM_LEV]>("PECND", m_num_elems);
  m_phi = ExecViewManaged<Real * [NP][NP][NUM_LEV]>("PHI", m_num_elems);
  m_derived_un0 = ExecViewManaged<Real * [NP][NP][NUM_LEV]>(
      "Derived Lateral Velocity 1", m_num_elems);
  m_derived_vn0 = ExecViewManaged<Real * [NP][NP][NUM_LEV]>(
      "Derived Lateral Velocity 2", m_num_elems);

  m_u = ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>(
      "Lateral Velocity 1", m_num_elems);
  m_v = ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>(
      "Lateral Velocity 2", m_num_elems);
  m_t = ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>(
      "Temperature", m_num_elems);
  m_dp3d = ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>(
      "DP3D", m_num_elems);

  m_qdp = ExecViewManaged<Real * [Q_NUM_TIME_LEVELS][QSIZE_D][NP][NP][NUM_LEV]>(
      "qdp", m_num_elems);
  m_eta_dot_dpdn =
...
void CaarRegion::random_init(const int num_elems, std::mt19937_64 &engine) {
  ...
  ExecViewManaged<Real * [NP][NP][NUM_LEV]>::HostMirror h_omega_p =
      Kokkos::create_mirror_view(m_omega_p);
  ExecViewManaged<Real * [NP][NP][NUM_LEV]>::HostMirror h_pecnd =
      Kokkos::create_mirror_view(m_pecnd);
  ExecViewManaged<Real * [NP][NP][NUM_LEV]>::HostMirror h_phi =
      Kokkos::create_mirror_view(m_phi);
  ExecViewManaged<Real * [NP][NP][NUM_LEV]>::HostMirror h_derived_un0 =
      Kokkos::create_mirror_view(m_derived_un0);
  ExecViewManaged<Real * [NP][NP][NUM_LEV]>::HostMirror h_derived_vn0 =
      Kokkos::create_mirror_view(m_derived_vn0);

  ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>::HostMirror h_u =
      Kokkos::create_mirror_view(m_u);
  ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>::HostMirror h_v =
      Kokkos::create_mirror_view(m_v);
  ExecViewManaged<Real * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>::HostMirror h_t =
...
void CaarRegion::push_3d(F90Ptr &derived_phi, F90Ptr &derived_pecnd,
  ...
  for (int ie = 0, k_3d_scalars = 0, k_3d_vectors = 0; ie < m_num_elems; ++ie) {
    for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
      for (int igp = 0; igp < NP; ++igp) {
        for (int jgp = 0; jgp < NP; ++jgp, ++k_3d_scalars) {
          derived_omega_p[k_3d_scalars] = h_omega_p(ie, igp, jgp, ilev);
          derived_pecnd[k_3d_scalars] = h_pecnd(ie, igp, jgp, ilev);
          derived_phi[k_3d_scalars] = h_phi(ie, igp, jgp, ilev);
        }
      }

      for (int igp = 0; igp < NP; ++igp) {
        for (int jgp = 0; jgp < NP; ++jgp, ++k_3d_vectors) {
          derived_v[k_3d_vectors] = h_derived_un0(ie, igp, jgp, ilev);
        }
      }
      for (int igp = 0; igp < NP; ++igp) {
        for (int jgp = 0; jgp < NP; ++jgp, ++k_3d_vectors) {
          derived_v[k_3d_vectors] = h_derived_vn0(ie, igp, jgp, ilev);
        }
      }
    }
  }
  ...
  for (int ie = 0, k_4d_scalars = 0, k_4d_vectors = 0; ie < m_num_elems; ++ie) {
    for (int tl = 0; tl < NUM_TIME_LEVELS; ++tl) {
      for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp, ++k_4d_scalars) {
            state_dp3d[k_4d_scalars] = h_dp3d(ie, tl, igp, jgp, ilev);
            state_t[k_4d_scalars] = h_t(ie, tl, igp, jgp, ilev);
          }
        }

        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp, ++k_4d_vectors) {
            state_v[k_4d_vectors] = h_u(ie, tl, igp, jgp, ilev);
          }
        }
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp, ++k_4d_vectors) {
            state_v[k_4d_vectors] = h_v(ie, tl, igp, jgp, ilev);
          }
        }
      }
    }
    ...
    for (int qni = 0; qni < Q_NUM_TIME_LEVELS; ++qni) {
      for (int iq = 0; iq < QSIZE_D; ++iq) {
        for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
          for (int igp = 0; igp < NP; ++igp) {
            for (int jgp = 0; jgp < NP; ++jgp, ++k_qdp) {
              state_qdp[k_qdp] = h_qdp(ie, qni, iq, igp, jgp, ilev);
            }
          }
        }
      }

...
std::pair<int, int> team_num_threads_vectors_from_pool(
  ...
  const int num_threads_per_element =
      ((num_parallel_iterations > 0 && pool_size > num_parallel_iterations)
...
std::pair<int, int> team_num_threads_vectors_for_gpu(
  ...
  const int num_warps = std::max(
      min_num_warps,
      std::min(max_num_warps, ((num_parallel_iterations > 0 &&
  ...
  if (tp.prefer_threads) {
    const int num_threads =
        ((tp.max_threads_usable > num_device_threads) ? num_device_threads
                                                      : tp.max_threads_usable);
    return std::make_pair(num_threads,
                          prevpow2(num_device_threads / num_threads));
  } else {
    const int num_vectors = prevpow2(
        (tp.max_vectors_usable > num_device_threads) ? num_device_threads
...
std::pair<int, int>
DefaultThreadsDistribution<Hommexx_Cuda>::team_num_threads_vectors(
    const int num_parallel_iterations, const ThreadPreferences tp) {
...
// this. For now:
  ...
  const int max_num_warps =
      16;  // Kokkos::Impl::cuda_internal_maximum_grid_count();
#endif
#else
  ...
  return Parallel::team_num_threads_vectors_for_gpu(
      num_warps_device, num_threads_warp, min_num_warps, max_num_warps,

...
void prim_step(const Real dt, const bool compute_diagnostics) {
  ...
  if (params.use_semi_lagrangian_transport) {
    Errors::option_error("prim_step", "use_semi_lagrangian_transport",
  ...
  GPTLstart("tl-s deep_copy+derived_dp");
  {
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy<ExecSpace>(
                         ...
                         KOKKOS_LAMBDA(const int idx) {
                           ...
                           derived_dp(ie, igp, jgp, ilev) =
                               dp3d(ie, tl.n0, igp, jgp, ilev);
                         });
  }
  ExecSpace::fence();
  ...
  // For rsplit=0:
  //   if tracer scheme needs v on lagrangian levels it has to vertically
  //   interpolate if tracer scheme needs dp3d, it needs to derive it from ps_v
  // ===============
  // Advect tracers if their count is > 0.
  // not be advected.  This will be cleaned up when the physgrid is merged into
  // CAM trunk Currently advecting all species

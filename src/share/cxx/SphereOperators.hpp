...
class SphereOperators {
 ...
 public:
  ...
  void allocate_buffers(
      const Kokkos::TeamPolicy<ExecSpace, Tags...> &team_policy) {
    ...
    const int alloc_dim = OnGpu<ExecSpace>::value
                              ...
                              : std::min(get_num_concurrent_teams(team_policy),
                                         num_parallel_iterations);
  ...
  KOKKOS_INLINE_FUNCTION void gradient_sphere_sl(
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, grad_iters), [&](const int loop_idx) {
  ...
  KOKKOS_INLINE_FUNCTION void gradient_sphere_update_sl(
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, grad_iters), [&](const int loop_idx) {
          ...
          grad_s(0, i, j) +=
              D_inv(0, 0, i, j) * tmp0 + D_inv(0, 1, i, j) * tmp1;
          grad_s(1, i, j) +=
              D_inv(1, 0, i, j) * tmp0 + D_inv(1, 1, i, j) * tmp1;
        });
    kv.team_barrier();
  }

  KOKKOS_INLINE_FUNCTION void divergence_sphere_sl(
      const KernelVariables &kv,
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          gv_buf(0, igp, jgp) =
              (D_inv(0, 0, igp, jgp) * v0 + D_inv(1, 0, igp, jgp) * v1) *
              metdet(igp, jgp);
          gv_buf(1, igp, jgp) =
              (D_inv(0, 1, igp, jgp) * v0 + D_inv(1, 1, igp, jgp) * v1) *
              metdet(igp, jgp);
        });
    kv.team_barrier();
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, div_iters), [&](const int loop_idx) {
          const int igp = loop_idx / NP;
          const int jgp = loop_idx % NP;
  ...
  KOKKOS_INLINE_FUNCTION void vorticity_sphere_sl(
      const KernelVariables &kv, const ExecViewUnmanaged<const Real[NP][NP]> u,
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          vcov_buf(0, igp, jgp) =
              D(0, 0, igp, jgp) * u_ij + D(0, 1, igp, jgp) * v_ij;
          vcov_buf(1, igp, jgp) =
              D(1, 0, igp, jgp) * u_ij + D(1, 1, igp, jgp) * v_ij;
        });
    kv.team_barrier();

    constexpr int vort_iters = NP * NP;
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, vort_iters), [&](const int loop_idx) {
          const int igp = loop_idx / NP;
          const int jgp = loop_idx % NP;
          Real dudy = 0.0;
          Real dvdx = 0.0;
  ...
  KOKKOS_INLINE_FUNCTION void gradient_sphere(
    ...
    static_assert(
        NUM_LEV_REQUEST > 0,
        "Error! Template argument NUM_LEV_REQUEST must be positive.\n");
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          const int jgp = loop_idx % NP;
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV_REQUEST),
              [&](const int &ilev) {
                Scalar v0, v1;
                for (int kgp = 0; kgp < NP; ++kgp) {
                ...
                grad_s(0, igp, jgp, ilev) =
                    D_inv(0, 0, igp, jgp) * v0 + D_inv(0, 1, igp, jgp) * v1;
                grad_s(1, igp, jgp, ilev) =
                    D_inv(1, 0, igp, jgp) * v0 + D_inv(1, 1, igp, jgp) * v1;
              });
        });
    kv.team_barrier();
  }
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                grad_s(0, igp, jgp, ilev) +=
                    D_inv(0, 0, igp, jgp) * dsdx + D_inv(0, 1, igp, jgp) * dsdy;
                grad_s(1, igp, jgp, ilev) +=
                    D_inv(1, 0, igp, jgp) * dsdx + D_inv(1, 1, igp, jgp) * dsdy;
              });
        });
    kv.team_barrier();
  }
        ...
        Kokkos::TeamThreadRange(kv.team, div_iters), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                div_v(igp, jgp, ilev) =
                    (dudx + dvdy) *
                    (1.0 / metdet(igp, jgp) * PhysicalConstants::rrearth);
              });
        });
    kv.team_barrier();
  }
  ...
  template <int NUM_LEV_REQUEST = NUM_LEV>
  KOKKOS_INLINE_FUNCTION void divergence_sphere_update(
      const KernelVariables &kv, const Real alpha,
        ...
        Kokkos::TeamThreadRange(kv.team, div_iters), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                const Scalar qtensijk0 =
                    add_hyperviscosity ? qtens(igp, jgp, ilev) : 0;
                qtens(igp, jgp, ilev) =
                    (qdp(igp, jgp, ilev) +
                     alpha * ((dudx + dvdy) * (1.0 / metdet(igp, jgp) *
                                               PhysicalConstants::rrearth)) +
                     qtensijk0);
              });
        });
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                vcov_buf(0, igp, jgp, ilev) =
                    D(0, 0, igp, jgp) * u_ijk + D(0, 1, igp, jgp) * v_ijk;
                vcov_buf(1, igp, jgp, ilev) =
                    D(1, 0, igp, jgp) * u_ijk + D(1, 1, igp, jgp) * v_ijk;
              });
        });
    kv.team_barrier();
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                const auto &v0 = v(0, igp, jgp, ilev);
                const auto &v1 = v(1, igp, jgp, ilev);
                sphere_buf(0, igp, jgp, ilev) =
                    D(0, 0, igp, jgp) * v0 + D(0, 1, igp, jgp) * v1;
                sphere_buf(1, igp, jgp, ilev) =
                    D(1, 0, igp, jgp) * v0 + D(1, 1, igp, jgp) * v1;
              });
        });
    kv.team_barrier();
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                for (int jgp = 0; jgp < NP; ++jgp) {
                  ...
                  dd -= (spheremp(ngp, jgp) * v(0, ngp, jgp, ilev) *
                             dvv(jgp, mgp) +
                         spheremp(jgp, mgp) * v(1, jgp, mgp, ilev) *
                             dvv(jgp, ngp)) *
                        PhysicalConstants::rrearth;
                }
  ...
  // but without if-statements for hypervis_power, var_coef, and
  // hypervis_scaling.  for 2d fields, there should be either laplace_simple, or
  // laplace_tensor for the whole run.
  template <int NUM_LEV_REQUEST = NUM_LEV>
  KOKKOS_INLINE_FUNCTION void laplace_tensor(
    ...
    gradient_sphere<NUM_LEV_REQUEST>(kv, field, grad_s);
    // now multiply tensorVisc(:,:,i,j)*grad_s(i,j) (matrix*vector, independent
    // of i,j )  but it requires a temp var to store a result. the result is then
    // placed to grad_s,  or should it be an extra temp var instead of an extra
    // loop?
    constexpr int num_iters = NP * NP;
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, num_iters), [&](const int loop_idx) {
          const int igp = loop_idx / NP;
          const int jgp = loop_idx % NP;
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV_REQUEST),
              [&](const int &ilev) {
                ...
                sphere_buf(0, igp, jgp, ilev) =
                    tensorVisc(0, 0, igp, jgp) * grad_s0 +
                    tensorVisc(1, 0, igp, jgp) * grad_s1;
                sphere_buf(1, igp, jgp, ilev) =
                    tensorVisc(0, 1, igp, jgp) * grad_s0 +
                    tensorVisc(1, 1, igp, jgp) * grad_s1;
              });
        });
    kv.team_barrier();
  ...
  template <int NUM_LEV_REQUEST = NUM_LEV>
  KOKKOS_INLINE_FUNCTION void curl_sphere_wk_testcov(
      const KernelVariables &kv,
      const ExecViewUnmanaged<const Scalar[NP][NP][NUM_LEV]> scalar,
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                curls(0, igp, jgp, ilev) =
                    (D(0, 0, igp, jgp) * sb0 + D(1, 0, igp, jgp) * sb1) *
                    PhysicalConstants::rrearth;
                curls(1, igp, jgp, ilev) =
  ...
  // This computes curls = alpha*curl(scalar) + beta*curls, where scalar is the
  // input view, and curls is the output view
  template <int NUM_LEV_REQUEST = NUM_LEV>
  KOKKOS_INLINE_FUNCTION void curl_sphere_wk_testcov_update(
      const KernelVariables &kv, const Real alpha, const Real beta,
      const ExecViewUnmanaged<const Scalar[NP][NP][NUM_LEV]> scalar,
      const ExecViewUnmanaged<Scalar[2][NP][NP][NUM_LEV]> curls) const {
    static_assert(
        NUM_LEV_REQUEST > 0,
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                curls(0, ngp, mgp, ilev) =
                    beta * curls(0, ngp, mgp, ilev) +
                    alpha *
                        (D(0, 0, ngp, mgp) * sb0 + D(1, 0, ngp, mgp) * sb1) *
                        PhysicalConstants::rrearth;
                curls(1, ngp, mgp, ilev) =
                    beta * curls(1, ngp, mgp, ilev) +
                    alpha *
  ...
  template <int NUM_LEV_REQUEST = NUM_LEV>
  KOKKOS_INLINE_FUNCTION void grad_sphere_wk_testcov(
      const KernelVariables &kv,
      const ExecViewUnmanaged<const Scalar[NP][NP][NUM_LEV]> scalar,
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                grads(0, ngp, mgp, ilev) =
                    (D(0, 0, ngp, mgp) * b0 + D(1, 0, ngp, mgp) * b1) *
                    PhysicalConstants::rrearth;
                grads(1, ngp, mgp, ilev) =
                    (D(0, 1, ngp, mgp) * b0 + D(1, 1, ngp, mgp) * b1) *
                    PhysicalConstants::rrearth;
              });
        });
    kv.team_barrier();
  ...
  template <int NUM_LEV_REQUEST = NUM_LEV>
  KOKKOS_INLINE_FUNCTION void vlaplace_sphere_wk_cartesian(
      const KernelVariables &kv,
      const ExecViewUnmanaged<const Real[2][2][NP][NP]> tensorVisc,
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                laplace0(igp, jgp, ilev) = vec_sph2cart(0, 0, igp, jgp) * v0 +
                                           vec_sph2cart(1, 0, igp, jgp) * v1;
                laplace1(igp, jgp, ilev) = vec_sph2cart(0, 1, igp, jgp) * v0 +
                                           vec_sph2cart(1, 1, igp, jgp) * v1;
                laplace2(igp, jgp, ilev) = vec_sph2cart(0, 2, igp, jgp) * v0 +
                                           vec_sph2cart(1, 2, igp, jgp) * v1;
              });
        });
    kv.team_barrier();

    // Use laplace* as input, and then overwrite it with the output (saves
    // temporaries)
...
#define UNDAMPRRCART
#ifdef UNDAMPRRCART
                laplace(0, igp, jgp, ilev) =
                    vec_sph2cart(0, 0, igp, jgp) * laplace0(igp, jgp, ilev) +
                ...
                laplace(1, igp, jgp, ilev) =
                    vec_sph2cart(1, 0, igp, jgp) * laplace0(igp, jgp, ilev) +
  ...
  template <int NUM_LEV_REQUEST = NUM_LEV>
  KOKKOS_INLINE_FUNCTION void vlaplace_sphere_wk_contra(
      const KernelVariables &kv, const Real nu_ratio,
      const ExecViewUnmanaged<const Scalar[2][NP][NP][NUM_LEV]> vector,
      const ExecViewUnmanaged<Scalar[2][NP][NP][NUM_LEV]> laplace) const {
    static_assert(
        NUM_LEV_REQUEST > 0,
    ...
    if (nu_ratio > 0 && nu_ratio != 1.0) {
      Kokkos::parallel_for(
          ...
          [&](const int loop_idx) {
            ...
            Kokkos::parallel_for(
                Kokkos::ThreadVectorRange(kv.team, NUM_LEV_REQUEST),
                [&](const int &ilev) { div(igp, jgp, ilev) *= nu_ratio; });
          });
      kv.team_barrier();
    }
    ...
    curl_sphere_wk_testcov_update<NUM_LEV_REQUEST>(kv, -1.0, 1.0, vort,
                                                   grad_curl_cov);
        ...
        Kokkos::TeamThreadRange(kv.team, np_squared), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int &ilev) {
                ...
                laplace(0, igp, jgp, ilev) =
                    f * vector(0, igp, jgp, ilev) * re2 +
                    grad_curl_cov(0, igp, jgp, ilev);
                laplace(1, igp, jgp, ilev) =
                    f * vector(1, igp, jgp, ilev) * re2 +
                    grad_curl_cov(1, igp, jgp, ilev);
              });
        });
    kv.team_barrier();
  ...
  ExecViewManaged<Scalar * [NUM_3D_SCALAR_BUFFERS][NP][NP][NUM_LEV]>
      scalar_buf_ml;
  ExecViewManaged<Scalar * [NUM_3D_VECTOR_BUFFERS][2][NP][NP][NUM_LEV]>
      vector_buf_ml;

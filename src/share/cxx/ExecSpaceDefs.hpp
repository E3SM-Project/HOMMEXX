...
std::pair<int, int> team_num_threads_vectors_from_pool(
...
std::pair<int, int> team_num_threads_vectors_for_gpu(
...
static typename std::enable_if<!OnGpu<ExecSpaceType>::value, int>::type
get_num_concurrent_teams(
    const Kokkos::TeamPolicy<ExecSpaceType, Tags...> &policy) {
...
using MDRangePolicy = Kokkos::Experimental::MDRangePolicy<
    ExecutionSpace,
    Kokkos::Experimental::Rank<Rank, Kokkos::Experimental::Iterate::Right,
...
struct Memory {
  ...
  KOKKOS_INLINE_FUNCTION static Scalar *get_shmem(
      const typename Kokkos::TeamPolicy<ExeSpace>::member_type &,
      const size_t sz = 0) {
    return nullptr;
  }
...
struct Memory<Hommexx_Cuda> {
  ...
  KOKKOS_INLINE_FUNCTION static Scalar *get_shmem(
    ...
    return static_cast<Scalar *>(
        team.team_shmem().get_shmem(n * sizeof(Scalar)));
  }
...
struct Dispatch {
  ...
  KOKKOS_FORCEINLINE_FUNCTION void parallel_reduce(
      const Kokkos::TeamPolicy<ExecSpace>::member_type &team,
      const LoopBdyType &loop_boundaries, const Lambda &lambda,
      ValueType &result) {
  ...
  static KOKKOS_FORCEINLINE_FUNCTION void parallel_for_NP2(
      const typename Kokkos::TeamPolicy<ExeSpace>::member_type &team,
      const Lambda &lambda) {
    VECTOR_IVDEP_LOOP
    VECTOR_SIMD_LOOP
    for (int k = 0; k < NP * NP; ++k) lambda(k);
  }

  template <class Lambda, typename ValueType>
  static KOKKOS_FORCEINLINE_FUNCTION void parallel_reduce_NP2(
      const typename Kokkos::TeamPolicy<ExeSpace>::member_type &team,
      const Lambda &lambda, ValueType &result) {
    result = ValueType();
    for (int k = 0; k < NP * NP; ++k) lambda(k, result);
  }

  template <class Lambda>
  static KOKKOS_FORCEINLINE_FUNCTION void parallel_scan(
      const typename Kokkos::TeamPolicy<ExeSpace>::member_type &team,
...
struct Dispatch<Kokkos::Cuda> {
  ...
  template <typename LoopBdyType, class Lambda, typename ValueType>
  KOKKOS_FORCEINLINE_FUNCTION static void parallel_reduce(
  ...
  static KOKKOS_FORCEINLINE_FUNCTION void parallel_reduce_NP2(
      const Kokkos::TeamPolicy<Kokkos::Cuda>::member_type &team,
      const Lambda &lambda, ValueType &result) {
    parallel_reduce(team, Kokkos::ThreadVectorRange(team, NP * NP), lambda,
                    result);
  }
  ...
  static KOKKOS_FORCEINLINE_FUNCTION void parallel_scan(
    ...
    using value_type = typename Kokkos::Impl::FunctorAnalysis<
        Kokkos::Impl::FunctorPatternInterface::SCAN, void, Lambda>::value_type;

    // All threads init result.
    value_type accumulator = value_type();
    // Only one thread does the work, i.e., only one sweeps (so last arg to
    // lambda is true)
    Kokkos::single(Kokkos::PerThread(team), [&]() {

...
HyperviscosityFunctor::HyperviscosityFunctor(const SimulationParams &params,
...
HyperviscosityFunctor::~HyperviscosityFunctor() {
  // This empty destructor (where HyperviscosityFunctorImpl type is completely
  // known) is necessary for pimpl idiom to work with unique_ptr. The issue is
  // the deleter, which needs to know the size of the stored type, and which
  // would be called from the implicitly declared default destructor, which
  // would be in the header file, where HyperviscosityFunctorImpl type is
  // incomplete.
}
...
void HyperviscosityFunctor::run(const int np1, const Real dt,

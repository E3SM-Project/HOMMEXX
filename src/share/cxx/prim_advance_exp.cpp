...
void u3_5stage_timestep(const int nm1, const int n0, const int np1,
                        const int n0_qdp, const Real dt, const Real eta_ave_w,
                        const bool compute_diagnostics);

// -------------- IMPLEMENTATIONS -------------- //

void prim_advance_exp(const int nm1, const int n0, const int np1, const Real dt,
                      const bool compute_diagnostics) {
  GPTLstart("tl-ae prim_advance_exp");
  // Get simulation params
  SimulationParams &params = Context::singleton().get_simulation_params();

  // Note: In the following, all the checks are superfluous, since we already
  // check that
  //       the options are supported when we init the simulation params.
  //       However, this way we remind ourselves that in these cases there is
  //       some missing code to convert from Fortran
  ...
  if (params.prescribed_wind) {
    Errors::runtime_abort(
        "'prescribed wind' functionality not yet available in C++ build.\n",
  ...
  switch (method) {
    case 5:
      // Perform RK stages
      u3_5stage_timestep(nm1, n0, np1, tl.n0_qdp, dt, eta_ave_w,
                         compute_diagnostics);
      break;
    default: {
      std::string msg = "[prim_advance_exp_iter]:";
  ...
  if (compute_diagnostics) {
    Errors::runtime_abort(
        "'compute diagnostic' functionality not yet available in C++ build.\n",
        Errors::err_not_implemented);
  }
#endif
  ...
  if (params.time_step_type == 0) {
    ...
    Errors::runtime_abort(msg, Errors::err_not_implemented);
    // call
    // advance_hypervis_lf(edge3p1,elem,hvcoord,hybrid,deriv,nm1,n0,np1,nets,nete,dt_vis)

  } else if (params.time_step_type <= 10) {
    // Get and run the HVF
    HyperviscosityFunctor &functor =
        Context::singleton().get_hyperviscosity_functor();
  ...
  // Compute (5u1-u0)/4 and store it in timelevel nm1
  {
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<ExecSpace>(
            0, elements.num_elems() * NP * NP * NUM_LEV),
        KOKKOS_LAMBDA(const int it) {
          ...
          t(ie, nm1, igp, jgp, ilev) =
              (5.0 * t(ie, nm1, igp, jgp, ilev) - t(ie, n0, igp, jgp, ilev)) /
              4.0;
          v(ie, nm1, 0, igp, jgp, ilev) = (5.0 * v(ie, nm1, 0, igp, jgp, ilev) -
                                           v(ie, n0, 0, igp, jgp, ilev)) /
                                          4.0;
          v(ie, nm1, 1, igp, jgp, ilev) = (5.0 * v(ie, nm1, 1, igp, jgp, ilev) -
                                           v(ie, n0, 1, igp, jgp, ilev)) /
                                          4.0;
          dp3d(ie, nm1, igp, jgp, ilev) = (5.0 * dp3d(ie, nm1, igp, jgp, ilev) -
                                           dp3d(ie, n0, igp, jgp, ilev)) /
                                          4.0;
        });
  }
  ExecSpace::fence();

  // Stage 5: u5 = (5u1-u0)/4 + 3dt/4 RHS(u4), t_rhs = t + dt/5 + dt/5 + dt/3 +
  // 2dt/3

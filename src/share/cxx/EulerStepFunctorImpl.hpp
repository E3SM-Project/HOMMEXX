...
template <>
struct reduction_identity<Real2> {
...
struct SerialLimiter {
  template <int limiter_option, typename ArrayGll, typename ArrayGllLvl,
            typename Array2Lvl>
  KOKKOS_INLINE_FUNCTION static void run(const ArrayGll &sphweights,
                                         const ArrayGllLvl &idpmass,
                                         const Array2Lvl &iqlim,
                                         const ArrayGllLvl &iptens,
...
class EulerStepFunctorImpl {
 ...
 public:
  ...
  void reset(const SimulationParams &params) {
    ...
    if (m_data.limiter_option == 4) {
      ...
      msg +=
          "The program should have errored out earlier though. Plese, "
          "investigate.";
  ...
  void init_boundary_exchanges() {
    ...
    {
      auto bm_exchange_minmax =
          Context::singleton().get_buffers_manager(MPI_EXCHANGE_MIN_MAX);
  ...
  /*
    ...
    ! nu_p=0:    qtens_biharmonic *= dp0                   (apply viscosity only
    to q) ! nu_p>0):   qtens_biharmonc *= elem()%psdiss_ave      (for
    consistency, if nu_p=nu_q)
   */
  void compute_biharmonic_pre() {
    ...
    if (m_data.nu_p > 0) {
      ...
      Kokkos::parallel_for(
          Homme::get_default_team_policy<ExecSpace, BIHPreNoNup>(
              m_elements.num_elems() * m_data.qsize),
          *this);
    }
  ...
  void compute_biharmonic_post() {
    ...
    if (m_data.consthv) {
      Kokkos::parallel_for(
          Homme::get_default_team_policy<ExecSpace, BIHPostConstHV>(
              m_elements.num_elems() * m_data.qsize),
          *this);
    } else {
      Kokkos::parallel_for(
          Homme::get_default_team_policy<ExecSpace, BIHPostTensorHV>(
              m_elements.num_elems() * m_data.qsize),
          *this);
    }
    ExecSpace::fence();
  ...
  void dpdiss_adjustment(KernelVariables &kv, const TeamMember &team) const {
    const auto qtens_biharmonic =
        Homme::subview(m_tracers.qtens_biharmonic, kv.ie, kv.iq);
    const auto dpdiss_ave =
        Homme::subview(m_elements.m_derived_dpdiss_ave, kv.ie);
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(team, NP * NP), [&](const int loop_idx) {
          const int i = loop_idx / NP;
          const int j = loop_idx % NP;
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(team, NUM_LEV), [&](const int &k) {
                qtens_biharmonic(i, j, k) = qtens_biharmonic(i, j, k) *
                                            dpdiss_ave(i, j, k) /
                                            m_hvcoord.dp0(k);
              });
        });
  ...
  void rhsviss_adjustment(KernelVariables &kv, const TeamMember &team) const {
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(team, NP * NP), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(team, NUM_LEV), [&](const int &k) {
                qtens_biharmonic(i, j, k) =
                    (f * m_hvcoord.dp0(k) * qtens_biharmonic(i, j, k) /
                     spheremp(i, j));
              });
        });
  }
  ...
  void operator()(const PrecomputeDivDp &, const TeamMember &team) const {
    KernelVariables kv(team);
    m_sphere_ops.divergence_sphere(
        kv, Homme::subview(m_elements.m_derived_vn0, kv.ie),
        Homme::subview(m_elements.m_derived_divdp, kv.ie));
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(team, NP * NP), [&](const int idx) {
          const int igp = idx / NP;
          const int jgp = idx % NP;
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(team, NUM_LEV), [&](const int ilev) {
          ...
          Kokkos::parallel_for(
              ...
              [&](const int loop_idx) {
                ...
                Kokkos::parallel_for(
                    Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
                    [&](const int &k) {
                      //! derived variable divdp_proj() (DSS'd version of divdp)
                      //! will only be correct on 2nd and 3rd stage but that's
                      //! ok because rhs_multiplier=0 on the first stage:
                      // Store this in unused buffer.
                      buf(kv.ie, i, j, k) = dp(kv.ie, i, j, k) -
                                            rhsmdt * divdp_proj(kv.ie, i, j, k);
                    });
              });
        });
  }
  ...
  void compute_qmin_qmax() {
    ...
    Kokkos::TeamPolicy<ExecSpace> policy(num_parallel_iterations, tv.first,
                                         tv.second);
    Kokkos::parallel_for(policy, KOKKOS_LAMBDA(const TeamMember &team) {
      KernelVariables kv(team, qsize);
      const auto dp_t = Homme::subview(dp, kv.ie);
      const auto qdp_t = Homme::subview(qdp, kv.ie, n0_qdp, kv.iq);
      const auto qtens_biharmonic_t =
          Homme::subview(qtens_biharmonic, kv.ie, kv.iq);
  ...
  void neighbor_minmax_finish() { m_mm_be->recv_and_unpack_min_max(); }
 ...
 private:
  ...
  void compute_2d_advection_step(const KernelVariables &kv) const {
    ...
    const auto &f_dss =
        (c.DSSopt == DSSOption::ETA
             ? e.m_eta_dot_dpdn
             : c.DSSopt == DSSOption::OMEGA ? e.m_omega_p
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int loop_idx) {
          ...
          const int j = loop_idx % NP;
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&](const int &k) {
                const auto dp = e.buffers.pressure(kv.ie, i, j, k);
                e.buffers.vstar(kv.ie, 0, i, j, k) =
                    e.m_derived_vn0(kv.ie, 0, i, j, k) / dp;
                e.buffers.vstar(kv.ie, 1, i, j, k) =
                    e.m_derived_vn0(kv.ie, 1, i, j, k) / dp;
                if (lim_quasi_monotone) {
                  //! Note that the term dpdissk is independent of Q
                  //! UN-DSS'ed dp at timelevel n0+1:
                  e.buffers.dpdissk(kv.ie, i, j, k) =
                      dp - c.dt * e.m_derived_divdp(kv.ie, i, j, k);
                  if (add_ps_diss) {
                    //! add contribution from UN-DSS'ed PS dissipation
                    //!          dpdiss(:,:) = ( hvcoord%hybi(k+1) -
                    //!          hvcoord%hybi(k) ) *
                    //!          elem(ie)%derived%psdiss_biharmonic(:,:)
                    e.buffers.dpdissk(kv.ie, i, j, k) +=
                        diss_fac *
                        e.m_derived_dpdiss_biharmonic(kv.ie, i, j, k) /
                        e.m_spheremp(kv.ie, i, j);
                  }
                }
                //! also DSS extra field
                //! note: eta_dot_dpdn is actually dimension nlev+1, but nlev+1
                //! data is all zero so we only have to DSS 1:nlev
  ...
  void apply_spheremp(const KernelVariables &kv) const {
    const auto qdp =
        Homme::subview(m_tracers.qdp, kv.ie, m_data.np1_qdp, kv.iq);
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int loop_idx) {
          ...
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
                               [&](const int &ilev) {
                                 qdp(igp, jgp, ilev) =
                                     spheremp(igp, jgp) * qtens(igp, jgp, ilev);
                               });
        });
  }
  ...
  template <typename Limit, typename ArrayGll, typename ArrayGllLvl,
            typename Array2Lvl>
  KOKKOS_INLINE_FUNCTION static void with_limiter_shell(
      const TeamMember &team, const Limit &limit, const ArrayGll &sphweights,
      const ArrayGllLvl &dpmass, const Array2Lvl &qlim,
      const ArrayGllLvl &ptens) {
    const int NP2 = NP * NP;
    ...
    const auto f = [&](const int ilev) {
      ...
      Real *const data =
          team_data ? team_data + 2 * NP2 * team.team_rank() : nullptr;
      ...
      Kokkos::Real2 sums;
      Dispatch<>::parallel_reduce_NP2(team,
                                      [&](const int &k, Kokkos::Real2 &sums) {
                                        sums.v[0] += x[k] * c[k];
                                        sums.v[1] += c[k];
                                      },
                                      sums);
      if (sums.v[1] <= 0)
        return;  //! this should never happen, but if it does, dont limit
      ...
      //! testing
      if (minp < 0) minp = qlim(0, vpi)[vsi] = 0;
    ...
    if (OnGpu<ExecSpace>::value || team.team_size() > 1) {
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team, NUM_PHYSICAL_LEV), f);
    } else {
      VECTOR_IVDEP_LOOP
      VECTOR_SIMD_LOOP
      for (int ilev = 0; ilev < NUM_PHYSICAL_LEV; ++ilev) f(ilev);
    }
  }
 ...
 public:  // Expose for unit testing.
  ...
  KOKKOS_INLINE_FUNCTION static void limiter_optim_iter_full(
      const TeamMember &team, const ArrayGll &sphweights,
      const ArrayGllLvl &dpmass, const Array2Lvl &qlim,
      const ArrayGllLvl &ptens) {
    struct Limit {
      KOKKOS_INLINE_FUNCTION void operator()(
          const TeamMember &team, const Real &mass, const Real &minp,
          const Real &maxp, Real *KOKKOS_RESTRICT const x,
        ...
        for (int iter = 0; iter < maxiter; ++iter) {
          ...
          if (std::abs(addmass) <= tol_limiter * std::abs(mass)) break;

          if (addmass > 0) {
            Real weightssum = 0;
            Dispatch<>::parallel_reduce_NP2(
                team,
                [&](const int &k, Real &weightssum) {
                  if (x[k] < maxp) weightssum += c[k];
                },
                weightssum);
            const auto adw = addmass / weightssum;
            Dispatch<>::parallel_for_NP2(
                team, [&](const int &k) { x[k] += (x[k] < maxp) ? adw : 0; });
          } else {
            Real weightssum = 0;
            Dispatch<>::parallel_reduce_NP2(
                team,
                [&](const int &k, Real &weightssum) {
                  if (x[k] > minp) weightssum += c[k];
                },
                weightssum);
            const auto adw = addmass / weightssum;
            Dispatch<>::parallel_for_NP2(
                team, [&](const int &k) { x[k] += (x[k] > minp) ? adw : 0; });
          }
        }
      }
    };
  ...
  template <typename ArrayGll, typename ArrayGllLvl, typename Array2Lvl>
  KOKKOS_INLINE_FUNCTION static void limiter_clip_and_sum(
      const TeamMember &team, const ArrayGll &sphweights,
      const ArrayGllLvl &dpmass, const Array2Lvl &qlim,
      const ArrayGllLvl &ptens) {
    struct Limit {
      KOKKOS_INLINE_FUNCTION void operator()(
        ...
        if (addmass > 0) {
          ...
          Dispatch<>::parallel_reduce_NP2(
              team,
              [&](const int &k, Real &fac) { fac += c[k] * (maxp - x[k]); },
              fac);
          if (fac > 0) {
            // Update.
            fac = addmass / fac;
            Dispatch<>::parallel_for_NP2(
                team, [&](const int &k) { x[k] += fac * (maxp - x[k]); });
          }
        } else {
          Real fac = 0;
          Dispatch<>::parallel_reduce_NP2(
              team,
              [&](const int &k, Real &fac) { fac += c[k] * (x[k] - minp); },
              fac);
          if (fac > 0) {
            fac = addmass / fac;
            Dispatch<>::parallel_for_NP2(
                team, [&](const int &k) { x[k] += fac * (x[k] - minp); });
          }
        }
      }
    };
...
template <int limiter_option, typename ArrayGll, typename ArrayGllLvl,
          typename Array2Lvl>
KOKKOS_INLINE_FUNCTION void SerialLimiter<ExecSpace>::run(
    const ArrayGll &sphweights, const ArrayGllLvl &idpmass,
...
#define forlev for (int lev = 0; lev < NUM_PHYSICAL_LEV; ++lev)
  ...
  ViewUnmanaged<const Real[NP][NP][NUM_LEV * VECTOR_SIZE]> dpmass(
      &idpmass(0, 0, 0)[0]);
  ViewUnmanaged<Real[NP][NP][NUM_LEV * VECTOR_SIZE]> x(&iptens(0, 0, 0)[0]),
      c(&irwrk(0, 0, 0)[0]);
  ViewUnmanaged<Real[2][NUM_LEV * VECTOR_SIZE]> qlim(&iqlim(0, 0)[0]);
  ...
  if (limiter_option == 8) {
    ...
    for (int iter = 0; iter < maxiter; ++iter) {
      ...
      forij {
        ...
        forlev {
          ...
          if (addmass[lev] <= 0) {
            if (x(i, j, lev) > qlim(0, lev)) f[lev] += c(i, j, lev);
          } else {
            if (x(i, j, lev) < qlim(1, lev)) f[lev] += c(i, j, lev);
          }
        }
      }
      ...
      VECTOR_SIMD_LOOP
      forlev {
        if (f[lev] != 0) f[lev] = addmass[lev] / f[lev];
      }
      ...
      forij {
        ...
        forlev {
          if (done[lev]) continue;
          if (addmass[lev] <= 0) {
            if (x(i, j, lev) > qlim(0, lev)) x(i, j, lev) += f[lev];
          } else {
            if (x(i, j, lev) < qlim(1, lev)) x(i, j, lev) += f[lev];
          }
        }
      }
    }
    ...
    forij {
      ...
      forlev {
        auto &xij = x(i, j, lev);
        if (addmass[lev] <= 0) {
          if (xij > qlim(0, lev)) f[lev] += c(i, j, lev) * (xij - qlim(0, lev));
        } else {
          if (xij < qlim(1, lev)) f[lev] += c(i, j, lev) * (qlim(1, lev) - xij);
        }
      }
    }
    ...
    forij {
      ...
      forlev {
        auto &xij = x(i, j, lev);
        if (addmass[lev] <= 0) {
          if (xij > qlim(0, lev)) xij += f[lev] * (xij - qlim(0, lev));
        } else {
          if (xij < qlim(1, lev)) xij += f[lev] * (qlim(1, lev) - xij);
        }
      }
    }
  } else {
  ...
  forij {
    VECTOR_IVDEP_LOOP
    VECTOR_SIMD_LOOP
    forlev { x(i, j, lev) *= dpmass(i, j, lev); }
  }

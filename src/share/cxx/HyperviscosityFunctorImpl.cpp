...
HyperviscosityFunctorImpl::HyperviscosityFunctorImpl(
    const SimulationParams &params, const Elements &elements,
    const Derivative &deriv)
    : m_elements(elements),
      m_deriv(deriv),
      m_data(params.hypervis_subcycle, 1.0, params.nu_top, params.nu,
  ...
  if (m_data.nu_top > 0) {
    ...
    Kokkos::Array<Real, NUM_BIHARMONIC_PHYSICAL_LEVELS> lev_nu_scale_top = {
        4.0, 2.0, 1.0};
    for (int phys_lev = 0; phys_lev < NUM_BIHARMONIC_PHYSICAL_LEVELS;
         ++phys_lev) {
...
void HyperviscosityFunctorImpl::run(const int np1, const Real dt,
                                    const Real eta_ave_w) {
  m_data.np1 = np1;
  m_data.dt = dt / m_data.hypervis_subcycle;
  m_data.eta_ave_w = eta_ave_w;

  Kokkos::RangePolicy<ExecSpace, TagUpdateStates> policy_update_states(
      0, m_elements.num_elems() * NP * NP * NUM_LEV);
...
void HyperviscosityFunctorImpl::biharmonic_wk_dp3d() const {
  // For the first laplacian we use a differnt kernel, which uses directly the
  // states at timelevel np1 as inputs. This way we avoid copying the states to
  // *tens buffers.

  auto policy_first_laplace =
      Homme::get_default_team_policy<ExecSpace, TagFirstLaplaceHV>(
          m_elements.num_elems());
  ...
  if (m_data.consthv) {
    auto policy_second_laplace =
        Homme::get_default_team_policy<ExecSpace, TagSecondLaplaceConstHV>(
            m_elements.num_elems());
    Kokkos::parallel_for(policy_second_laplace, *this);
  } else {
    auto policy_second_laplace =
        Homme::get_default_team_policy<ExecSpace, TagSecondLaplaceTensorHV>(
            m_elements.num_elems());
    Kokkos::parallel_for(policy_second_laplace, *this);
  }
  Kokkos::fence();

...
class HyperviscosityFunctorImpl {
  struct HyperviscosityData {
    HyperviscosityData(const int hypervis_subcycle_in, const Real nu_ratio_in,
                       const Real nu_top_in, const Real nu_in,
                       const Real nu_p_in, const Real nu_s_in,
                       const Real hypervis_scaling_in)
        : hypervis_subcycle(hypervis_subcycle_in),
          nu_ratio(nu_ratio_in),
          nu_top(nu_top_in),
          nu(nu_in),
          nu_p(nu_p_in),
          nu_s(nu_s_in),
 ...
 public:
  ...
  HyperviscosityFunctorImpl(const SimulationParams &params,
                            const Elements &elements, const Derivative &deriv);
  ...
  void operator()(const TagFirstLaplaceHV &, const TeamMember &team) const {
    ...
    m_sphere_ops.laplace_simple(
        kv, Homme::subview(m_elements.m_t, kv.ie, m_data.np1),
        Homme::subview(m_elements.buffers.ttens, kv.ie));
    // Laplacian of pressure
    m_sphere_ops.laplace_simple(
        kv, Homme::subview(m_elements.m_dp3d, kv.ie, m_data.np1),
        Homme::subview(m_elements.buffers.dptens, kv.ie));

    // Laplacian of velocity
    m_sphere_ops.vlaplace_sphere_wk_contra(
        kv, m_data.nu_ratio, Homme::subview(m_elements.m_v, kv.ie, m_data.np1),
        Homme::subview(m_elements.buffers.vtens, kv.ie));
  }

  // second iter of laplace, const hv
  KOKKOS_INLINE_FUNCTION
  void operator()(const TagSecondLaplaceConstHV &,
                  const TeamMember &team) const {
    KernelVariables kv(team);
    // Laplacian of temperature
    m_sphere_ops.laplace_simple(
        kv, Homme::subview(m_elements.buffers.ttens, kv.ie),
        Homme::subview(m_elements.buffers.ttens, kv.ie));
    // Laplacian of pressure
    m_sphere_ops.laplace_simple(
        kv, Homme::subview(m_elements.buffers.dptens, kv.ie),
        Homme::subview(m_elements.buffers.dptens, kv.ie));

    // Laplacian of velocity
    m_sphere_ops.vlaplace_sphere_wk_contra(
        kv, m_data.nu_ratio, Homme::subview(m_elements.buffers.vtens, kv.ie),
        Homme::subview(m_elements.buffers.vtens, kv.ie));
  }

  // second iter of laplace, tensor hv
  KOKKOS_INLINE_FUNCTION
  void operator()(const TagSecondLaplaceTensorHV &,
                  const TeamMember &team) const {
    KernelVariables kv(team);
    // Laplacian of temperature
    m_sphere_ops.laplace_tensor(
        kv, Homme::subview(m_elements.m_tensorvisc, kv.ie),
        Homme::subview(m_elements.buffers.ttens, kv.ie),
        Homme::subview(m_elements.buffers.ttens, kv.ie));
    // Laplacian of pressure
    m_sphere_ops.laplace_tensor(
        ...
        Homme::subview(m_elements.buffers.dptens, kv.ie));
    // Laplacian of velocity
    m_sphere_ops.vlaplace_sphere_wk_cartesian(
        kv, Homme::subview(m_elements.m_tensorvisc, kv.ie),
  ...
  void operator()(const TagUpdateStates &, const int idx) const {
    ...
    m_elements.buffers.vtens(ie, 0, igp, jgp, ilev) =
        (m_data.dt * m_elements.buffers.vtens(ie, 0, igp, jgp, ilev) *
         m_elements.m_rspheremp(ie, igp, jgp));
    m_elements.buffers.vtens(ie, 1, igp, jgp, ilev) =
        (m_data.dt * m_elements.buffers.vtens(ie, 1, igp, jgp, ilev) *
         m_elements.m_rspheremp(ie, igp, jgp));
    m_elements.m_v(ie, m_data.np1, 0, igp, jgp, ilev) +=
        m_elements.buffers.vtens(ie, 0, igp, jgp, ilev);
    m_elements.m_v(ie, m_data.np1, 1, igp, jgp, ilev) +=
        m_elements.buffers.vtens(ie, 1, igp, jgp, ilev);

    m_elements.buffers.ttens(ie, igp, jgp, ilev) =
        (m_data.dt * m_elements.buffers.ttens(ie, igp, jgp, ilev) *
         m_elements.m_rspheremp(ie, igp, jgp));
    const Scalar heating =
        m_elements.buffers.vtens(ie, 0, igp, jgp, ilev) *
            m_elements.m_v(ie, m_data.np1, 0, igp, jgp, ilev) +
        m_elements.buffers.vtens(ie, 1, igp, jgp, ilev) *
            m_elements.m_v(ie, m_data.np1, 1, igp, jgp, ilev);
    ...
    m_elements.m_dp3d(ie, m_data.np1, igp, jgp, ilev) =
        (m_elements.buffers.dptens(ie, igp, jgp, ilev) *
         m_elements.m_rspheremp(ie, igp, jgp));
  }
  ...
  void operator()(const TagHyperPreExchange, const TeamMember &team) const {
    KernelVariables kv(team);
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int &point_idx) {
          const int igp = point_idx / NP;
          const int jgp = point_idx % NP;
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&](const int &lev) {
                ...
                m_elements.m_derived_dpdiss_biharmonic(kv.ie, igp, jgp, lev) +=
                    m_data.eta_ave_w *
                    m_elements.buffers.dptens(kv.ie, igp, jgp, lev) /
                    m_data.hypervis_subcycle;
              });
        });
    kv.team_barrier();
    ...
    constexpr int NUM_BIHARMONIC_LEV =
        (NUM_BIHARMONIC_PHYSICAL_LEVELS + VECTOR_SIZE - 1) / VECTOR_SIZE;
    ...
    Kokkos::parallel_for(
        Kokkos::TeamThreadRange(kv.team, NP * NP), [&](const int &point_idx) {
          ...
          if (m_data.nu_top > 0) {
            Kokkos::parallel_for(
                Kokkos::ThreadVectorRange(kv.team, int(NUM_BIHARMONIC_LEV)),
                [&](const int ilev) {
                  ...
                  m_elements.buffers.ttens(kv.ie, igp, jgp, ilev) +=
                      m_nu_scale_top[ilev] * laplace_t(kv.ie, igp, jgp, ilev);
          ...
          // While for T and v we exchange the tendencies, for dp3d we exchange
          // the updated state. However, since the BE structure already has
          // registerd the *tens quantities, we store the updated state in
          // dptens.
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV), [&](const int &lev) {

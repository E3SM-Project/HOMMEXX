...
CaarFunctor::CaarFunctor()
    : m_policy(Homme::get_default_team_policy<ExecSpace>(
          Context::singleton().get_elements().num_elems())) {
...
void CaarFunctor::init_boundary_exchanges(
...
void CaarFunctor::set_rk_stage_data(const int nm1, const int n0, const int np1,
  ...
  m_caar_impl->set_rk_stage_data(nm1, n0, np1, dt, eta_ave_w,
                                 compute_diagnostics);
}
...
void CaarFunctor::run() {
  ...
  Kokkos::parallel_for("caar loop pre-boundary exchange", m_policy,
                       *m_caar_impl);
  ExecSpace::fence();
...
void CaarFunctor::run(const int nm1, const int n0, const int np1, const Real dt,

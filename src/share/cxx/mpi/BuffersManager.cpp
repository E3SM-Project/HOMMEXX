...
BuffersManager::BuffersManager()
  ...
  // The "fake" buffers used for MISSING connections. These do not depend on the
  // requirements from the custormers, so we can create them right away.
  constexpr size_t blackhole_buffer_size = 2 * NUM_LEV * VECTOR_SIZE;
  m_blackhole_send_buffer =
      ExecViewManaged<Real *>("blackhole array", blackhole_buffer_size);
  m_blackhole_recv_buffer =
      ExecViewManaged<Real *>("blackhole array", blackhole_buffer_size);
...
void BuffersManager::set_connectivity(
...
bool BuffersManager::check_views_capacity(const int num_1d_fields,
                                          const int num_2d_fields,
                                          const int num_3d_fields) const {
  size_t mpi_buffer_size, local_buffer_size;
  required_buffer_sizes(num_1d_fields, num_2d_fields, num_3d_fields,
                        mpi_buffer_size, local_buffer_size);
...
void BuffersManager::allocate_buffers() {
  ...
  // Tell to all our customers that they need to redo the setup of the internal
  // buffer views
  for (auto &be_ptr : m_customers) {
    // Invalidate buffer views and requests in the customer (if none built yet,
    // it's a no-op)
...
void BuffersManager::add_customer(BoundaryExchange *add_me) {
  ...
  // If this customer has already started the registration, we can already
  // update the buffers sizes
...
void BuffersManager::update_requested_sizes(
    typename std::map<BoundaryExchange *, CustomerNeeds>::value_type
        &customer) {
  ...
  required_buffer_sizes(num_1d_fields, num_2d_fields, num_3d_fields,
                        customer.second.mpi_buffer_size,
                        customer.second.local_buffer_size);
...
void BuffersManager::required_buffer_sizes(const int num_1d_fields,
                                           const int num_2d_fields,
  ...
  // Note: for 2d/3d fields, we have 1 Real per GP (per level, in 3d). For 1d
  // fields,
  //       we have 2 Real per level (max and min over element).
  int elem_buf_size[2];
  elem_buf_size[etoi(ConnectionKind::CORNER)] =
      num_1d_fields * 2 * NUM_LEV * VECTOR_SIZE +
      (num_2d_fields + num_3d_fields * NUM_LEV * VECTOR_SIZE) * 1;
  elem_buf_size[etoi(ConnectionKind::EDGE)] =
      num_1d_fields * 2 * NUM_LEV * VECTOR_SIZE +
      (num_2d_fields + num_3d_fields * NUM_LEV * VECTOR_SIZE) * NP;

  // Compute the requested buffers sizes and compare with stored ones
  mpi_buffer_size += elem_buf_size[etoi(ConnectionKind::CORNER)] *
                     m_connectivity->get_num_connections<HostMemSpace>(
                         ConnectionSharing::SHARED, ConnectionKind::CORNER);
  mpi_buffer_size += elem_buf_size[etoi(ConnectionKind::EDGE)] *
                     m_connectivity->get_num_connections<HostMemSpace>(
                         ConnectionSharing::SHARED, ConnectionKind::EDGE);

  local_buffer_size += elem_buf_size[etoi(ConnectionKind::CORNER)] *
                       m_connectivity->get_num_connections<HostMemSpace>(
                           ConnectionSharing::LOCAL, ConnectionKind::CORNER);
  local_buffer_size += elem_buf_size[etoi(ConnectionKind::EDGE)] *
                       m_connectivity->get_num_connections<HostMemSpace>(

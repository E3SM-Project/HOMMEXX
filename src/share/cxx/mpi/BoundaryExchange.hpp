 ...
 *
 * This class (BE) takes care of exchanging the values of one or more fields in
 * the GP on the boundary of the elements with the neighboring elements. In
 * particular, it takes care of packing the values into some buffers, performing
 * the exchange (usually, this includes some MPI calls, unless running in serial
 * mode), and then unpacking the values, accumulating them into the receiving
 * elements. This process can be done for an arbitrary number of 2d fields (that
 * is, no vertical levels) and 3d fields (with vertical levels). If you have a
 ...
 *
 * This class relies on the BuffersManager (BM) class for the handling of the
 * buffers. See BuffersManager header for more info on that. As explained above,
...
class BoundaryExchange {
 public:
  BoundaryExchange();
  BoundaryExchange(std::shared_ptr<Connectivity> connectivity,
  ...
  // These number refers to *scalar* fields. A 2-vector field counts as 2
  // fields.
  void set_num_fields(const int num_1d_fields, const int num_2d_fields,
                      const int num_3d_fields);

  // Clean up MPI stuff and registered fields (but leaves connectivity and
  // buffers manager)
  void clean_up();
  ...
  // Note: num_dims is the # of dimensions to exchange, while start_dim is the
  // first to exchange
  template <int DIM, typename... Properties>
  void register_field(ExecView<Real * [DIM][NP][NP], Properties...> field,
                      int num_dims, int start_dim);
  template <int DIM, typename... Properties>
  void register_field(
      ExecView<Scalar * [DIM][NP][NP][NUM_LEV], Properties...> field,
      int num_dims, int start_dim);
  template <typename... Properties>
  void register_field(ExecView<Scalar[NP][NP][NUM_LEV], Properties...> field,
                      int ie, int iq);

  // Note: the outer dimension MUST be sliced, while the inner dimension can be
  // fully exchanged
  template <int OUTER_DIM, int DIM, typename... Properties>
  void register_field(
      ExecView<Scalar * [OUTER_DIM][DIM][NP][NP][NUM_LEV], Properties...> field,
      int idim_out, int num_dims, int start_dim);
  ...
  template <int DIM, typename... Properties>
  void register_min_max_fields(
      ExecView<Scalar * [DIM][2][NUM_LEV], Properties...> field_min_max,
      int num_dims, int start_dim);
  ...
  // Exchange all registered 1d fields, performing min/max operations with
  // neighbors
  ...
  struct Pointer {
    Pointer &operator=(const ptr_type &p) {
      ptr = p;
      return *this;
    }
  ...
  // Perform the pack_and_send and recv_and_unpack for boundary exchange of
  // 2d/3d fields
  void pack_and_send();
  void recv_and_unpack();

  // Perform the pack_and_send and recv_and_unpack for min/max boundary exchange
  // of 1d fields
  void pack_and_send_min_max();
  void recv_and_unpack_min_max();

  // If you are really not sure whether we are still transmitting, you can make
  // sure we're done by calling this
  void waitall();
 ...
 private:
  ...
  // This class contains all the buffers to be stuffed in the buffers views, and
  // used in pack/unpack, as well as the mpi buffers used in MPI calls (which
  // are the same as the former if MPIMemSpace=ExecMemSpace), and the blackhole
  // buffers (used for missing connections)
  ...
  // three buffers in the buffers manager. In particular, if it is a local
  // connection, it will point to local_buffer (on both send and recv views),
  // while for shared connection, it will point to the corresponding mpi buffer,
  // and for missing connection, it will point to the send/recv blackhole.

  ExecViewManaged<ExecViewUnmanaged<Scalar[2][NUM_LEV]> * * [NUM_CONNECTIONS]>
      m_send_1d_buffers;
  ExecViewManaged<ExecViewUnmanaged<Scalar[2][NUM_LEV]> * * [NUM_CONNECTIONS]>
      m_recv_1d_buffers;

  ExecViewManaged<ExecViewUnmanaged<Real *> * * [NUM_CONNECTIONS]>
      m_send_2d_buffers;
  ExecViewManaged<ExecViewUnmanaged<Real *> * * [NUM_CONNECTIONS]>
      m_recv_2d_buffers;

  ExecViewManaged<ExecViewUnmanaged<Scalar * [NUM_LEV]> * * [NUM_CONNECTIONS]>
      m_send_3d_buffers;
  ExecViewManaged<ExecViewUnmanaged<Scalar * [NUM_LEV]> * * [NUM_CONNECTIONS]>
      m_recv_3d_buffers;
  ...
  // The following flags are used to ensure that a bad user does not call
  // setup/cleanup/registration methods of this class in an order that generate
  // errors. And if he/she does, we try to avoid errors.
  ...
  void init_slot_idx_to_elem_conn_pair(
      std::vector<int> &h_slot_idx_to_elem_conn_pair, std::vector<int> &pids,
      std::vector<int> &pids_os);
  void free_requests();
...
void BoundaryExchange::register_field(
...

  {
    ...
    Kokkos::parallel_for(
        1, KOKKOS_LAMBDA(const int) { l_3d_fields(ie, iq) = field; });
  }
  // Temporary hack
...
template <int DIM, typename... Properties>
void BoundaryExchange::register_field(
    ExecView<Real * [DIM][NP][NP], Properties...> field, int num_dims,
    int start_dim) {
  using Kokkos::ALL;
...

  {
    ...
    Kokkos::parallel_for(
        MDRangePolicy<ExecSpace, 2>(
            {0, 0}, {m_connectivity->get_num_local_elements(), num_dims},
            {1, 1}),
        KOKKOS_LAMBDA(const int ie, const int idim) {
          l_2d_fields(ie, l_num_2d_fields + idim) =
              Kokkos::subview(field, ie, start_dim + idim, ALL, ALL);
        });
  }
...

  {
    ...
    Kokkos::parallel_for(
        MDRangePolicy<ExecSpace, 2>(
            ...
            {1, 1}),
        KOKKOS_LAMBDA(const int ie, const int idim) {
          l_3d_fields(ie, l_num_3d_fields + idim) =
              Kokkos::subview(field, ie, start_dim + idim, ALL, ALL, ALL);
        });
  }
...
void BoundaryExchange::register_field(
    ExecView<Scalar * [OUTER_DIM][DIM][NP][NP][NUM_LEV], Properties...> field,
    int outer_dim, int num_dims, int start_dim) {
  using Kokkos::ALL;

  // Sanity checks
  assert(m_registration_started && !m_registration_completed);
  assert(num_dims > 0 && start_dim >= 0 && outer_dim >= 0 && DIM > 0 &&
         OUTER_DIM > 0);
  assert(start_dim + num_dims <= DIM);
  assert(m_num_3d_fields + num_dims <= m_3d_fields.extent_int(1));
  assert(m_num_1d_fields == 0);

  {
    ...
    Kokkos::parallel_for(
        MDRangePolicy<ExecSpace, 2>(
            ...
            {1, 1}),
        KOKKOS_LAMBDA(const int ie, const int idim) {
          l_3d_fields(ie, l_num_3d_fields + idim) = Kokkos::subview(
              field, ie, outer_dim, start_dim + idim, ALL, ALL, ALL);
        });
  }
...
template <typename... Properties>
void BoundaryExchange::register_field(
    ExecView<Real * [NP][NP], Properties...> field) {
  using Kokkos::ALL;
...

  {
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy<ExecSpace>(
                             0, m_connectivity->get_num_local_elements()),
                         KOKKOS_LAMBDA(const int ie) {
                           l_2d_fields(ie, l_num_2d_fields) =
                               Kokkos::subview(field, ie, ALL, ALL);
                         });
  }
...
void BoundaryExchange::register_field(
...

  {
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy<ExecSpace>(
                             0, m_connectivity->get_num_local_elements()),
                         KOKKOS_LAMBDA(const int ie) {
                           l_3d_fields(ie, l_num_3d_fields) =
                               Kokkos::subview(field, ie, ALL, ALL, ALL);
                         });
  }
...

  {
    ...
    Kokkos::parallel_for(
        MDRangePolicy<ExecSpace, 2>(
            ...
            {1, 1}),
        KOKKOS_LAMBDA(const int ie, const int idim) {
          l_1d_fields(ie, l_num_1d_fields + idim) =
              Kokkos::subview(field_min_max, ie, start_dim + idim, ALL, ALL);
        });
  }

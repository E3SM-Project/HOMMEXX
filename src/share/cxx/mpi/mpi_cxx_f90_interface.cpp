...
void add_connection(const int &first_elem_lid, const int &first_elem_gid,
  ...
  if (first_elem_lid <= 0 || first_elem_gid <= 0 || first_elem_pos <= 0 ||
      first_elem_pid <= 0 || second_elem_lid <= 0 || second_elem_gid <= 0 ||
      second_elem_pos <= 0 || second_elem_pid <= 0) {
    std::cout << "ERROR! We were assuming F90 indices started at 1, but it "
                 "appears there is an exception.\n";
    std::abort();
  }
  ...
  Connectivity &connectivity = *Context::singleton().get_connectivity();
  connectivity.add_connection(first_elem_lid - 1, first_elem_gid - 1, fep,
                              first_elem_pid - 1, second_elem_lid - 1,
                              second_elem_gid - 1, sep, second_elem_pid - 1);
}

...
Connectivity::Connectivity()
    : m_finalized(false), m_initialized(false), m_num_local_elements(-1) {
...
void Connectivity::set_num_elements(const int num_local_elements) {
  // We don't allow to change the number of elements once set. There may be
  // downstream classes that already read num_elements from this class, and
  // would not be informed of the change. Besides, does it really make sense?
  // Does it add an interesting feature? I don't think so.
  ...
  m_connections = ExecViewManaged<ConnectionInfo * [NUM_CONNECTIONS]>(
      "Connections", m_num_local_elements);
  h_connections = Kokkos::create_mirror_view(m_connections);

  m_num_connections = ExecViewManaged<
      int[NUM_CONNECTION_SHARINGS + 1][NUM_CONNECTION_KINDS + 1]>(
      "Connections counts");
  ...
  //       missing connections! Everything else is set to invalid numbers (for
  //       safety)
...
void Connectivity::add_connection(
    const int first_elem_lid, const int first_elem_gid,
  ...
  if (first_elem_pid == m_comm.rank()) {
#ifndef NDEBUG
    // There is no edge-to-corner connection. Either the elements share a corner
    // or an edge.
    assert(m_helpers.CONNECTION_KIND[first_elem_pos] ==
           m_helpers.CONNECTION_KIND[second_elem_pos]);
    ...
    // Direction
    info.direction =
        etoi(m_helpers.CONNECTION_DIRECTION[local.pos][remote.pos]);
...
void Connectivity::finalize() {
  // Sanity check: Homme does not allow less than 2*2 elements on each of the
  // cube's faces, so each element should have at most ONE missing connection
  constexpr int corners[NUM_CORNERS] = {
  ...
  for (int ie = 0; ie < m_num_local_elements; ++ie) {
    ...
    for (int ic : corners) {
      if (h_connections(ie, ic).kind == etoi(ConnectionKind::MISSING)) {
        ...
        h_num_connections(2, 2) +=
            1;  // etoi(ConnectionSharing::MISSING),etoi(ConnectionKind::MISSING))
                // += 1;
      }
    }
  ...
  for (int kind = 0; kind < NUM_CONNECTION_KINDS; ++kind) {
    for (int sharing = 0; sharing < NUM_CONNECTION_SHARINGS; ++sharing) {
      h_num_connections(etoi(ConnectionSharing::ANY), kind) +=
          h_num_connections(sharing, kind);
      h_num_connections(sharing, etoi(ConnectionKind::ANY)) +=
          h_num_connections(sharing, kind);
    }
    h_num_connections(etoi(ConnectionKind::ANY), etoi(ConnectionKind::ANY)) +=
        h_num_connections(etoi(ConnectionSharing::ANY), kind);
  }

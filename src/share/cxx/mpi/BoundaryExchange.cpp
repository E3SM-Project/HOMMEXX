...
BoundaryExchange::BoundaryExchange(
    std::shared_ptr<Connectivity> connectivity,
...
void BoundaryExchange::set_connectivity(
    std::shared_ptr<Connectivity> connectivity) {
  // Functionality only available before registration starts
  assert(!m_registration_started && !m_registration_completed);

  // Make sure it is a valid connectivity (does not need to be
  // initialized/finalized yet) Also, replacing the connectivity could have
  // unintended side-effects; better prohibit it. Besides, when can it be
  // useful?
  assert(connectivity && !m_connectivity);

  // If the buffers manager is set and it stores a connectivity, it must match
  // the input one
  assert(!m_buffers_manager ||
         m_buffers_manager->get_connectivity() == connectivity);
...
void BoundaryExchange::set_buffers_manager(
    std::shared_ptr<BuffersManager> buffers_manager) {
  ...
  // If the buffers manager stores a connectivity, and we already have one set,
  // they must match
  assert(!buffers_manager->is_connectivity_set() || !(m_connectivity) ||
         buffers_manager->get_connectivity() == m_connectivity);
  ...
  // If I don't store a connectivity, take it from the buffers manager (if it
  // has one)
  if (m_buffers_manager->is_connectivity_set() && !m_connectivity) {
...
void BoundaryExchange::set_num_fields(const int num_1d_fields,
                                      const int num_2d_fields,
                                      const int num_3d_fields) {
  // We don't allow to call this method twice in a row. If you want to change
  // the number of fields, you need to call clean_up first, to get a fresh new
  // BoundaryExchange. Note: if you do call clean_up and then again
  // set_num_field and the new number of fields
  //       are smaller than the previous ones, BuffersManager will NOT shrink
  //       the buffers, so you may be left with buffers that are larger than
  //       what you need.
  assert(m_cleaned_up);

  // Make sure the connectivity is valid: must at least be a valid pointer, and
  // be initialized, i.e., store a valid number of elements, but may be
  // finalized later (before registration_completed call though)
  assert(m_connectivity && m_connectivity->is_initialized());

  // We strongly advocate for not using the same BE object for both 'standard'
  // exchange and min/max exchange
  assert(!(num_1d_fields > 0 && (num_2d_fields > 0 || num_2d_fields > 0)));

  // Note: we do not set m_num_1d_fields, m_num_2d_fields and m_num_3d_fields,
  // since we will use them as
  //       progressive indices while adding fields. Then, during
  //       registration_completed, we will check that they match the 2nd
  //       dimension of m_1d_fields, m_2d_fields and m_3d_fields.
...
void BoundaryExchange::registration_completed() {
  ...
  if (m_registration_completed) {
    // TODO: should we prohibit two consecutive calls of this method? It seems
    // harmless, so I'm allowing it
    return;
  }

  // TODO: should we assert that m_registration_started=true? Or simply return
  // if not? Can calling this
  //       method without a call to registration started be dangerous? Not
  //       sure...

  // At this point, the connectivity MUST be finalized already, and the buffers
  // manager must be set already
  assert(m_connectivity && m_connectivity->is_finalized());
  assert(m_buffers_manager);

  // Create the MPI data types, for corners and edges
  // Note: this is the size per element, per connection. It is the number of
  // Real's to send/receive to/from the neighbor Note: for 2d/3d fields, we have
  // 1 Real per GP (per level, in 3d). For 1d fields,
  //       we have 2 Real per level (max and min over element).
  m_elem_buf_size[etoi(ConnectionKind::CORNER)] =
      m_num_1d_fields * 2 * NUM_LEV * VECTOR_SIZE +
      (m_num_2d_fields + m_num_3d_fields * NUM_LEV * VECTOR_SIZE) * 1;
  m_elem_buf_size[etoi(ConnectionKind::EDGE)] =
      m_num_1d_fields * 2 * NUM_LEV * VECTOR_SIZE +
      (m_num_2d_fields + m_num_3d_fields * NUM_LEV * VECTOR_SIZE) * NP;
...
void BoundaryExchange::exchange() { exchange(nullptr); }

void BoundaryExchange::exchange(
    ExecViewUnmanaged<const Real * [NP][NP]> rspheremp) {
  exchange(&rspheremp);
}
...
void BoundaryExchange::exchange(
  ...
  // Check that this object is setup to perform exchange and not
  // exchange_min_max
  ...
  // If this is the first time we call the exchange method, or if the
  // BuffersManager has performed a reallocation since the last time this method
  // was called, we need to rebuild all our internal buffer views
  if (!m_buffer_views_and_requests_built) {
    build_buffer_views_and_requests();
  }

  // Hey, if some process can already send me stuff while I'm still packing,
  // that's ok
  if (!m_recv_requests.empty())
    HOMMEXX_MPI_CHECK_ERROR(
        MPI_Startall(m_recv_requests.size(), m_recv_requests.data()),
...
void BoundaryExchange::exchange_min_max() {
  ...
  // Check that this object is setup to perform exchange_min_max and not
  // exchange
...
void BoundaryExchange::pack_and_send() {
  ...
  // If this is the first time we call this method, or if the BuffersManager has
  // performed a reallocation since the last time this method was called, AND we
  // are calling this method manually, without relying on the exchange method to
  // call it, then we need to rebuild all our internal buffer views
  if (!m_buffer_views_and_requests_built) {
  ...
  if (m_num_2d_fields > 0) {
    ...
    Kokkos::parallel_for(
        MDRangePolicy<ExecSpace, 3>(
            {0, 0, 0}, {m_num_elems, NUM_CONNECTIONS, m_num_2d_fields},
            {1, 1, 1}),
        KOKKOS_LAMBDA(const int ie, const int iconn, const int ifield) {
          ...
          const LidGidPos &buffer_lidpos =
              info.sharing == etoi(ConnectionSharing::LOCAL) ? info.remote
                                                             : info.local;

          // Note: if it is an edge and the remote edge is in the reverse order,
          // we read the field_lidpos points backwards
          const auto &pts =
              helpers.CONNECTION_PTS[info.direction][field_lidpos.pos];
          for (int k = 0; k < helpers.CONNECTION_SIZE[info.kind]; ++k) {
            send_2d_buffers(buffer_lidpos.lid, ifield, buffer_lidpos.pos)(k) =
                fields_2d(field_lidpos.lid, ifield)(pts[k].ip, pts[k].jp);
          }
        });
  }
    ...
    if (OnGpu<ExecSpace>::value) {
      ...
      Kokkos::parallel_for(
          Kokkos::RangePolicy<ExecSpace>(
              0, m_num_elems * m_num_3d_fields * NUM_CONNECTIONS * NUM_LEV),
          KOKKOS_LAMBDA(const int it) {
            const int ie = it / (num_3d_fields * NUM_CONNECTIONS * NUM_LEV);
            const int ifield =
                (it / (NUM_CONNECTIONS * NUM_LEV)) % num_3d_fields;
            const int iconn = (it / NUM_LEV) % NUM_CONNECTIONS;
            const int ilev = it % NUM_LEV;
            const ConnectionInfo &info = connections(ie, iconn);
            const LidGidPos &field_lidpos = info.local;
            // For the buffer, in case of local connection, use remote info. In
            // fact, while with shared connections the mpi call will take care
            // of "copying" data to the remote recv buffer in the correct remote
            // element lid, for local connections we need to manually copy on
            // the remote element lid. We can do it here
            const LidGidPos &buffer_lidpos =
                info.sharing == etoi(ConnectionSharing::LOCAL) ? info.remote
                                                               : info.local;

            // Note: if it is an edge and the remote edge is in the reverse
            // order, we read the field_lidpos points backwards
            const auto &pts =
                helpers.CONNECTION_PTS[info.direction][field_lidpos.pos];
            const auto &sb =
                send_3d_buffers(buffer_lidpos.lid, ifield, buffer_lidpos.pos);
      ...
      const auto policy = Kokkos::TeamPolicy<ExecSpace>(num_parallel_iterations,
      ...
      Kokkos::parallel_for(policy, KOKKOS_LAMBDA(const TeamMember &team) {
        ...
        for (int iconn = 0; iconn < 8; ++iconn) {
          ...
          const auto &f3 = fields_3d(field_lidpos.lid, ifield);
          Kokkos::parallel_for(
              Kokkos::TeamThreadRange(kv.team,
              ...
              [&](const int &k) {
                ...
                Kokkos::parallel_for(
                    ...
                    [&](const int &ilev) { sbp[ilev] = f3p[ilev]; });
              });
        }
      });
    }
  ...
  m_buffers_manager->sync_send_buffer(this);  // Deep copy send_buffer into
                                              // mpi_send_buffer (no op if MPI
                                              // is on device)
  tstop("be sync_send_buffer");
  tstart("be send");
  if (!m_send_requests.empty())
    HOMMEXX_MPI_CHECK_ERROR(
        MPI_Startall(m_send_requests.size(), m_send_requests.data()),
        m_connectivity->get_comm().mpi_comm());
...
void BoundaryExchange::recv_and_unpack() { recv_and_unpack(nullptr); }
...
void BoundaryExchange::recv_and_unpack(
  ...
  if (!m_recv_requests.empty())
    HOMMEXX_MPI_CHECK_ERROR(
        MPI_Waitall(m_recv_requests.size(), m_recv_requests.data(),
                    MPI_STATUSES_IGNORE),
  ...
  if (m_num_2d_fields > 0) {
    ...
    const ConnectionHelpers helpers;
    Kokkos::parallel_for(
        MDRangePolicy<ExecSpace, 2>({0, 0}, {m_num_elems, m_num_2d_fields},
                                    {1, 1}),
        KOKKOS_LAMBDA(const int ie, const int ifield) {
          ...
          for (int icorner : helpers.UNPACK_CORNERS_ORDER) {
            if (recv_2d_buffers(ie, ifield, icorner).size() > 0) {
              fields_2d(ie, ifield)(
                  helpers.CONNECTION_PTS_FWD[icorner][0].ip,
  ...
  if (m_num_3d_fields > 0) {
    ...
    if (OnGpu<ExecSpace>::value) {
      ...
      Kokkos::parallel_for(
          Kokkos::RangePolicy<ExecSpace>(
              0, m_num_elems * m_num_3d_fields * NUM_LEV),
          KOKKOS_LAMBDA(const int it) {
      ...
      if (rspheremp) {
        ...
        Kokkos::parallel_for(
            Kokkos::RangePolicy<ExecSpace>(
                0, m_num_elems * m_num_3d_fields * NP * NP * NUM_LEV),
            KOKKOS_LAMBDA(const int it) {
      ...
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(const TeamMember &team) {
            ...
            const int ifield = kv.iq;
            const auto &f3 = fields_3d(ie, ifield);
            const auto ef = [&](const int &iedge, const int &k, const int &ip,
                                const int &jp) {
              ...
              Kokkos::parallel_for(
                  Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
                  [&](const int &ilev) { f3p[ilev] += r3p[ilev]; });
            };
            for (int k = 0; k < NP; ++k) {
              ef(0, k, 0, k);
              ef(1, k, NP - 1, k);
            ...
            const auto cf = [&](const int &icorner, const int &ip,
                                const int &jp) {
  ...
  if (!m_send_requests.empty())
    HOMMEXX_MPI_CHECK_ERROR(
        MPI_Waitall(m_send_requests.size(), m_send_requests.data(),
                    MPI_STATUSES_IGNORE),
        m_connectivity->get_comm().mpi_comm());  // Wait for all data to arrive
  tstop("be waitall 2");
...
void BoundaryExchange::pack_and_send_min_max() {
  ...
  // If this is the first time we call this method, or if the BuffersManager has
  // performed a reallocation since the last time this method was called, AND we
  // are calling this method manually, without relying on the exchange_min_max
  // method to call it, then we need to rebuild all our internal buffer views
  if (!m_buffer_views_and_requests_built) {
    tstart("be build_buffer_views_and_requests");
    build_buffer_views_and_requests();
    tstop("be build_buffer_views_and_requests");
  ...
  // NOTE: all of these temporary copies are necessary because of the issue of
  // lambda function not
  ...
  if (OnGpu<ExecSpace>::value) {
    Kokkos::parallel_for(
        Kokkos::RangePolicy<ExecSpace>(
            0, m_num_elems * m_num_1d_fields * NUM_CONNECTIONS * NUM_LEV),
        KOKKOS_LAMBDA(const int it) {
          ...
          send_1d_buffers(buffer_lidpos.lid, ifield, buffer_lidpos.pos)(
              MAX_ID, ilev) = fields_1d(field_lidpos.lid, ifield)(MAX_ID, ilev);
          send_1d_buffers(buffer_lidpos.lid, ifield, buffer_lidpos.pos)(
              MIN_ID, ilev) = fields_1d(field_lidpos.lid, ifield)(MIN_ID, ilev);
        });
  } else {
    const auto num_parallel_iterations =
        m_num_elems * num_1d_fields * NUM_CONNECTIONS;
    ThreadPreferences tp;
...
void BoundaryExchange::recv_and_unpack_min_max() {
  ...
  m_buffers_manager->sync_recv_buffer(this);  // Deep copy mpi_recv_buffer into
                                              // recv_buffer (no op if MPI is on
                                              // device)
  ...
  if (OnGpu<ExecSpace>::value) {
    Kokkos::parallel_for(
        Kokkos::RangePolicy<ExecSpace>(0,
                                       m_num_elems * m_num_1d_fields * NUM_LEV),
        KOKKOS_LAMBDA(const int it) {
          ...
          for (int neighbor = 0; neighbor < NUM_CONNECTIONS; ++neighbor) {
            ...
            if (connections(ie, neighbor).kind ==
                etoi(ConnectionKind::MISSING)) {
              continue;
            }
            fields_1d(ie, ifield)(MAX_ID, ilev) =
                max(fields_1d(ie, ifield)(MAX_ID, ilev),
                    recv_1d_buffers(ie, ifield, neighbor)(MAX_ID, ilev));
            fields_1d(ie, ifield)(MIN_ID, ilev) =
                min(fields_1d(ie, ifield)(MIN_ID, ilev),
    ...
    Kokkos::parallel_for(policy, KOKKOS_LAMBDA(const TeamMember &team) {
      ...
      for (int iconn = 0; iconn < NUM_CONNECTIONS; ++iconn) {
        ...
        const auto &f1 = fields_1d(ie, ifield);
        {
          ...
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
              [&](const int &ilev) { f1p[ilev] = min(f1p[ilev], rbp[ilev]); });
        }
        {
          ...
          Kokkos::parallel_for(
              Kokkos::ThreadVectorRange(kv.team, NUM_LEV),
              [&](const int &ilev) { f1p[ilev] = max(f1p[ilev], rbp[ilev]); });
        }
      }
    });
  }
...
void BoundaryExchange::build_buffer_views_and_requests() {
  ...
  // Ask the buffer manager to check for reallocation and then proceed with the
  // allocation (if needed) Note: if BM already knows about our needs, and
  // buffers were already allocated, then
  ...
  //       After reserving the buffer portion, update the offset by a given
  //       increment, depending on info.kind:
  //         - increment[CORNER]  = m_elem_buf_size[CORNER)] = 1  *
  //         (m_num_2d_fields + NUM_LEV*VECTOR_SIZE m_num_3d_fields)
  //         - increment[EDGE]    = m_elem_buf_size[EDGE)]   = NP *
  //         (m_num_2d_fields + NUM_LEV*VECTOR_SIZE m_num_3d_fields)
  //         - increment[MISSING] = 0 (point to the same blackhole)
  // Note: m_blackhole_send will be written many times, but will never be read
  // from.
  //       Kind of like streaming to /dev/null. blackhole_recv will be read from
  //       sometimes (24 times, to be precise, one for each of the 3 corner
  //       connections on each of the cube's vertices), but it's never written
  //       into, so will always contain zeros (set by the constructor).
  ...
  // Since we access the manager many times, we may as well call lock once and
  // store the shared_ptr.
  auto buffers_manager = m_buffers_manager;

  using local_buf_ptr_type =
      decltype(buffers_manager->get_local_buffer().data());
  using local_buf_val_type =
      decltype(*buffers_manager->get_local_buffer().data());
  HostViewManaged<Pointer<local_buf_ptr_type, local_buf_val_type>[3]>
      h_all_send_buffers("");
  HostViewManaged<Pointer<local_buf_ptr_type, local_buf_val_type>[3]>
      h_all_recv_buffers("");

  h_all_send_buffers[etoi(ConnectionSharing::LOCAL)] =
      buffers_manager->get_local_buffer().data();
  h_all_send_buffers[etoi(ConnectionSharing::SHARED)] =
      buffers_manager->get_send_buffer().data();
  h_all_send_buffers[etoi(ConnectionSharing::MISSING)] =
      buffers_manager->get_blackhole_send_buffer().data();
  h_all_recv_buffers[etoi(ConnectionSharing::LOCAL)] =
      buffers_manager->get_local_buffer().data();
  h_all_recv_buffers[etoi(ConnectionSharing::SHARED)] =
      buffers_manager->get_recv_buffer().data();
  h_all_recv_buffers[etoi(ConnectionSharing::MISSING)] =
      buffers_manager->get_blackhole_recv_buffer().data();

  // Create buffer views
  m_send_1d_buffers = decltype(m_send_1d_buffers)("1d send buffer", m_num_elems,
                                                  m_num_1d_fields);
  m_recv_1d_buffers = decltype(m_recv_1d_buffers)("1d recv buffer", m_num_elems,
                                                  m_num_1d_fields);
  m_send_2d_buffers = decltype(m_send_2d_buffers)("2d send buffer", m_num_elems,
                                                  m_num_2d_fields);
  m_recv_2d_buffers = decltype(m_recv_2d_buffers)("2d recv buffer", m_num_elems,
                                                  m_num_2d_fields);
  m_send_3d_buffers = decltype(m_send_3d_buffers)("3d send buffer", m_num_elems,
                                                  m_num_3d_fields);
  m_recv_3d_buffers = decltype(m_recv_3d_buffers)("3d recv buffer", m_num_elems,
                                                  m_num_3d_fields);

  std::vector<int> slot_idx_to_elem_conn_pair, pids, pid_offsets;
  init_slot_idx_to_elem_conn_pair(slot_idx_to_elem_conn_pair, pids,
                                  pid_offsets);

  // NOTE: I wanted to do this setup in parallel, on the execution space, but
  // there
  //       is a reduction hidden. In particular, we need to access buf_offset
  //       atomically, so that it is not update while we are still using it. One
  //       solution would be to use Kokkos::atomic_fetch_add, but it may kill
  //       the concurrency. And given that we do not have a ton of concurrency
  //       in this setup phase, and given that it is precisely only a setup
  //       phase, we may as well do things serially on the host, then deep_copy
  //       back to device
  ...
  for (int k = 0; k < m_num_elems * NUM_CONNECTIONS; ++k) {
    ...
    const int iconn = slot_idx_to_elem_conn_pair[k] % NUM_CONNECTIONS;
    {
      ...
      for (int ifield = 0; ifield < m_num_1d_fields; ++ifield) {
        h_send_1d_buffers(local.lid, ifield, local.pos) =
            ExecViewUnmanaged<Scalar[2][NUM_LEV]>(reinterpret_cast<Scalar *>(
                send_buffer.get() + h_buf_offset[info.sharing]));
        h_recv_1d_buffers(local.lid, ifield, local.pos) =
            ExecViewUnmanaged<Scalar[2][NUM_LEV]>(reinterpret_cast<Scalar *>(
                recv_buffer.get() + h_buf_offset[info.sharing]));
        h_buf_offset[info.sharing] +=
            h_increment_1d[info.kind] * NUM_LEV * VECTOR_SIZE;
      }
      for (int ifield = 0; ifield < m_num_2d_fields; ++ifield) {
        h_send_2d_buffers(local.lid, ifield, local.pos) =
            ExecViewUnmanaged<Real *>(
                send_buffer.get() + h_buf_offset[info.sharing],
                helpers.CONNECTION_SIZE[info.kind]);
        h_recv_2d_buffers(local.lid, ifield, local.pos) =
            ExecViewUnmanaged<Real *>(
                recv_buffer.get() + h_buf_offset[info.sharing],
                helpers.CONNECTION_SIZE[info.kind]);
      ...
      for (int ifield = 0; ifield < m_num_3d_fields; ++ifield) {
        h_send_3d_buffers(local.lid, ifield, local.pos) =
            ExecViewUnmanaged<Scalar * [NUM_LEV]>(
                reinterpret_cast<Scalar *>(send_buffer.get() +
                                           h_buf_offset[info.sharing]),
                helpers.CONNECTION_SIZE[info.kind]);
        h_recv_3d_buffers(local.lid, ifield, local.pos) =
            ExecViewUnmanaged<Scalar * [NUM_LEV]>(
                reinterpret_cast<Scalar *>(recv_buffer.get() +
                                           h_buf_offset[info.sharing]),
                helpers.CONNECTION_SIZE[info.kind]);
        h_buf_offset[info.sharing] +=
            h_increment_3d[info.kind] * NUM_LEV * VECTOR_SIZE;
      }
    }
  }
...

  {
    ...
    MPIViewManaged<Real *>::pointer_type send_ptr =
        buffers_manager->get_mpi_send_buffer().data();
    MPIViewManaged<Real *>::pointer_type recv_ptr =
        buffers_manager->get_mpi_recv_buffer().data();
    int offset = 0;
    for (int ip = 0; ip < npids; ++ip) {
      ...
      HOMMEXX_MPI_CHECK_ERROR(
          MPI_Send_init(send_ptr + offset, count, MPI_DOUBLE, pids[ip],
                        m_exchange_type, mpi_comm, &m_send_requests[ip]),
          m_connectivity->get_comm().mpi_comm());
      HOMMEXX_MPI_CHECK_ERROR(
          MPI_Recv_init(recv_ptr + offset, count, MPI_DOUBLE, pids[ip],
                        m_exchange_type, mpi_comm, &m_recv_requests[ip]),
          m_connectivity->get_comm().mpi_comm());
      offset += count;
    }
  }
...
void BoundaryExchange ::free_requests() {
...
// order of slots.
void BoundaryExchange ::init_slot_idx_to_elem_conn_pair(
    std::vector<int> &slot_idx_to_elem_conn_pair, std::vector<int> &pids,
    std::vector<int> &pid_offsets) {
  struct IP {
    int i, ord, pid;
...
void BoundaryExchange::clear_buffer_views_and_requests() {
  // BuffersManager calls this method upon (re)allocation of buffers, so that
  // all its customers are forced to recompute their internal buffers views.
  // However, if the views were not yet built, we can skip this
  if (!m_buffer_views_and_requests_built) {
    return;
  }

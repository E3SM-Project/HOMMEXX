...
void Diagnostics::init(
    const int num_elems, F90Ptr &elem_state_q_ptr, F90Ptr &elem_accum_qvar_ptr,
  ...
  // F90 ptr to array (n1,n2,...,nK,nelemd) can be stuffed directly in an
  // unmanaged view with scalar type Real*[nK]...[n2][n1] (with runtime
  // dimension nelemd)
  h_Q = HostViewUnmanaged<Real * [QSIZE_D][NUM_PHYSICAL_LEV][NP][NP]>(
      elem_state_q_ptr, num_elems);
  h_Qvar = HostViewUnmanaged<Real * [4][QSIZE_D][NP][NP]>(elem_accum_qvar_ptr,
                                                          num_elems);
  h_Qmass = HostViewUnmanaged<Real * [4][QSIZE_D][NP][NP]>(elem_accum_qmass_ptr,
                                                           num_elems);
  h_Q1mass = HostViewUnmanaged<Real * [QSIZE_D][NP][NP]>(elem_accum_q1mass_ptr,
                                                         num_elems);
  h_IEner =
      HostViewUnmanaged<Real * [4][NP][NP]>(elem_accum_iener_ptr, num_elems);
  h_IEner_wet =
      HostViewUnmanaged<Real * [NP][NP]>(elem_accum_iener_wet_ptr, num_elems);
  h_KEner =
      HostViewUnmanaged<Real * [4][NP][NP]>(elem_accum_kener_ptr, num_elems);
  h_PEner =
      HostViewUnmanaged<Real * [4][NP][NP]>(elem_accum_pener_ptr, num_elems);
}
      ...
      for (int iq = 0; iq < params.qsize; ++iq) {
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp) {
            ...
            HostViewUnmanaged<Real[NUM_PHYSICAL_LEV]> qdp(
                &qdp_h(ie, t2_qdp, iq, igp, jgp, 0)[0]);
...
void Diagnostics::prim_energy_halftimes(const bool before_advance,
                                        const int ivar) {
  // Get simulation params
  SimulationParams &params = Context::singleton().get_simulation_params();
  assert(params.params_set);
    ...
    for (int igp = 0; igp < NP; ++igp) {
      for (int jgp = 0; jgp < NP; ++jgp) {
        ...
        for (int ilev = 0; ilev < NUM_LEV; ++ilev) {
          Scalar dpt1 = dhyai(ilev) * hvcoord.ps0 +
                        dhybi(ilev) * h_ps_v(ie, t1, igp, jgp);
          ...
          for (int ivec = 0; ivec <= vector_end; ++ivec) {
            if (params.use_cpstar) {
              Real qval_t1 =
                  qdp_h(ie, t1_qdp, 0, igp, jgp, ilev)[ivec] / dpt1[ivec];
            ...
            KEner += (std::pow(u(ilev)[ivec], 2) + std::pow(v(ilev)[ivec], 2)) *
                     0.5 * dpt1[ivec];

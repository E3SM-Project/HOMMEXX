...
void init_simulation_params_c(
    const int &remap_alg, const int &limiter_option, const int &rsplit,
    const int &qsplit, const int &time_step_type, const int &energy_fixer,
    const int &qsize, const int &state_frequency, const Real &nu,
    const Real &nu_p, const Real &nu_q, const Real &nu_s, const Real &nu_div,
    const Real &nu_top, const int &hypervis_order, const int &hypervis_subcycle,
    const double &hypervis_scaling, const bool &prescribed_wind,
    const bool &moisture, const bool &disable_diagnostics,
    const bool &use_cpstar, const bool &use_semi_lagrangian_transport) {
  // Check that the simulation options are supported. This helps us in the
  // future, since we are currently 'assuming' some option have/not have certain
  // values. As we support for more options in the C++ build, we will remove
  // some checks
  Errors::check_option("init_simulation_params_c", "vert_remap_q_alg",
                       remap_alg, {1, 2});
  Errors::check_option("init_simulation_params_c", "prescribed_wind",
                       prescribed_wind, {false});
  Errors::check_option("init_simulation_params_c", "hypervis_order",
                       hypervis_order, {2});
  Errors::check_option("init_simulation_params_c",
                       "use_semi_lagrangian_transport",
                       use_semi_lagrangian_transport, {false});
  Errors::check_option("init_simulation_params_c", "time_step_type",
                       time_step_type, {5});
  Errors::check_option("init_simulation_params_c", "limiter_option",
                       limiter_option, {8, 9});
  Errors::check_option("init_simulation_params_c", "nu_p", nu_p, 0.0,
                       Errors::ComparisonOp::GT);
  Errors::check_option("init_simulation_params_c", "nu", nu, 0.0,
                       Errors::ComparisonOp::GT);
  Errors::check_option("init_simulation_params_c", "nu_div", nu_div, 0.0,
                       Errors::ComparisonOp::GT);
  Errors::check_options_relation("init_simulation_params_c", "nu_div", "nu",
                                 nu_div, nu, Errors::ComparisonOp::EQ);
...
void init_hvcoord_c(const Real &ps0, CRCPtr &hybrid_am_ptr,
...
void cxx_push_results_to_f90(F90Ptr &elem_state_v_ptr,
                             F90Ptr &elem_state_temp_ptr,
                             F90Ptr &elem_state_dp3d_ptr,
                             F90Ptr &elem_state_Qdp_ptr, F90Ptr &elem_Q_ptr,
                             F90Ptr &elem_state_ps_v_ptr,
  ...
  // in an unmanaged view with scalar Real*[NUM_TIME_LEVELS][NP][NP] (with
  // runtime dimension nelemd)
...
void init_elements_2d_c(const int &num_elems, CF90Ptr &D, CF90Ptr &Dinv,
                        CF90Ptr &fcor, CF90Ptr &mp, CF90Ptr &spheremp,
                        CF90Ptr &rspheremp, CF90Ptr &metdet, CF90Ptr &metinv,
  ...
  r.init_2d(D, Dinv, fcor, mp, spheremp, rspheremp, metdet, metinv, phis,
            tensorvisc, vec_sph2cart, consthv);
}

void init_elements_states_c(CF90Ptr &elem_state_v_ptr,
                            CF90Ptr &elem_state_temp_ptr,
                            CF90Ptr &elem_state_dp3d_ptr,
                            CF90Ptr &elem_state_Qdp_ptr,
                            CF90Ptr &elem_state_ps_v_ptr) {
  Elements &elements = Context::singleton().get_elements();
  ...
  // in an unmanaged view with scalar Real*[NUM_TIME_LEVELS][NP][NP] (with
  // runtime dimension nelemd)
  HostViewUnmanaged<const Real * [NUM_TIME_LEVELS][NP][NP]> ps_v_f90(
      elem_state_ps_v_ptr, elements.num_elems());
...
void init_diagnostics_c(F90Ptr &elem_state_q_ptr, F90Ptr &elem_accum_qvar_ptr,
                        F90Ptr &elem_accum_qmass_ptr,
                        F90Ptr &elem_accum_q1mass_ptr,
                        F90Ptr &elem_accum_iener_ptr,
                        F90Ptr &elem_accum_iener_wet_ptr,
                        F90Ptr &elem_accum_kener_ptr,
                        F90Ptr &elem_accum_pener_ptr) {
  Elements &elements = Context::singleton().get_elements();
  Diagnostics &diagnostics = Context::singleton().get_diagnostics();

  diagnostics.init(elements.num_elems(), elem_state_q_ptr, elem_accum_qvar_ptr,
                   elem_accum_qmass_ptr, elem_accum_q1mass_ptr,
                   elem_accum_iener_ptr, elem_accum_iener_wet_ptr,

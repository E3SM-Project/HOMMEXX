...
template <typename ViewT, typename ArrayT>
struct exec_view_mappable {
  ...
  static constexpr bool value = Kokkos::Impl::ViewMapping<
      ...
      typename Kokkos::View<
          ArrayT, Kokkos::LayoutRight, ExecSpace,
...
struct host_view_mappable {
  ...
  static constexpr bool value = Kokkos::Impl::ViewMapping<
      typename ViewT::traits,
      typename Kokkos::View<
          ArrayT, Kokkos::LayoutRight, Kokkos::HostSpace,
          Kokkos::MemoryTraits<Kokkos::Unmanaged | Kokkos::Restrict> >::traits,
      void>::is_assignable;
#endif
};
      ...
      for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
        ...
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp) {
            dest(ie, tl, level, igp, jgp) =
                source_mirror(ie, tl, igp, jgp, ilev)[ivec];
          }
        }
      }
    }
...
typename std::enable_if<
    (exec_view_mappable<
         Source_T, Scalar * [NUM_TIME_LEVELS][2][NP][NP][NUM_LEV]>::value &&
     host_view_mappable<
  ...
  for (int ie = 0; ie < source.extent_int(0); ++ie) {
    for (int tl = 0; tl < NUM_TIME_LEVELS; ++tl) {
      for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
        ...
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp) {
            dest(ie, tl, level, 0, igp, jgp) =
                source_mirror(ie, tl, 0, igp, jgp, ilev)[ivec];
            dest(ie, tl, level, 1, igp, jgp) =
                source_mirror(ie, tl, 1, igp, jgp, ilev)[ivec];
          }
        }
      }
    }
...
template <typename Source_T, typename Dest_T>
typename std::enable_if<(exec_view_mappable<Source_T, Real * [NP][NP]>::value &&
      ...
      for (int igp = 0; igp < NP; ++igp) {
        for (int jgp = 0; jgp < NP; ++jgp) {
          dest(ie, level, 0, igp, jgp) =
              source_mirror(ie, 0, igp, jgp, ilev)[ivec];
          dest(ie, level, 1, igp, jgp) =
              source_mirror(ie, 1, igp, jgp, ilev)[ivec];
        }
      }
    }
  }
...
typename std::enable_if<
    (exec_view_mappable<Source_T, Scalar * [QSIZE_D][NP][NP][NUM_LEV]>::value &&
     host_view_mappable<Dest_T,
                        Real * [QSIZE_D][NUM_PHYSICAL_LEV][NP][NP]>::value),
    void>::type
sync_to_host(Source_T source, Dest_T dest) {
  typename Source_T::HostMirror source_mirror =
      Kokkos::create_mirror_view(source);
        ...
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp) {
            dest(ie, tracer, level, igp, jgp) =
                source_mirror(ie, tracer, igp, jgp, ilev)[ivec];
          }
        }
      }
    }
...
typename std::enable_if<
    (exec_view_mappable<
         Source_T,
         Scalar * [Q_NUM_TIME_LEVELS][QSIZE_D][NP][NP][NUM_LEV]>::value &&
     host_view_mappable<
         Dest_T,
         Real * [Q_NUM_TIME_LEVELS][QSIZE_D][NUM_PHYSICAL_LEV][NP][NP]>::value),
    void>::type
sync_to_host(Source_T source, Dest_T dest) {
  typename Source_T::HostMirror source_mirror =
      Kokkos::create_mirror_view(source);
    ...
    for (int time = 0; time < Q_NUM_TIME_LEVELS; ++time) {
      for (int tracer = 0; tracer < QSIZE_D; ++tracer) {
        for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
          ...
          for (int igp = 0; igp < NP; ++igp) {
            for (int jgp = 0; jgp < NP; ++jgp) {
              dest(ie, time, tracer, level, igp, jgp) =
                  source_mirror(ie, time, tracer, igp, jgp, ilev)[ivec];
            }
          }
        }
      }
...
// These last two sync_to_host deal with views with NUM_INTERFACE_LEV levels on
// host, and either copy the whole view to a view with NUM_LEV_P level packs
// (1st version) , or copy only the first NUM_PHYSICAL_LEV to a view with
// NUM_LEV level packs (2nd version)
template <typename Source_T, typename Dest_T>
typename std::enable_if<
...
typename std::enable_if<
    (host_view_mappable<
         Source_T, Real * [NUM_TIME_LEVELS][NUM_PHYSICAL_LEV][NP][NP]>::value &&
     exec_view_mappable<Dest_T,
                        Scalar * [NUM_TIME_LEVELS][NP][NP][NUM_LEV]>::value),
    void>::type
  ...
  for (int ie = 0; ie < source.extent_int(0); ++ie) {
    for (int tl = 0; tl < NUM_TIME_LEVELS; ++tl) {
      for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
        ...
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp) {
            dest_mirror(ie, tl, igp, jgp, ilev)[ivec] =
                source(ie, tl, level, igp, jgp);
          }
        }
      }
    }
...
template <typename Source_T, typename Dest_T>
typename std::enable_if<
    (host_view_mappable<Source_T, Real * [NUM_TIME_LEVELS][NUM_PHYSICAL_LEV][2]
                                                          [NP][NP]>::value &&
     exec_view_mappable<Dest_T,
                        Scalar * [NUM_TIME_LEVELS][2][NP][NP][NUM_LEV]>::value),
    void>::type
sync_to_device(Source_T source, Dest_T dest) {
  typename Dest_T::HostMirror dest_mirror = Kokkos::create_mirror_view(dest);
  for (int ie = 0; ie < source.extent_int(0); ++ie) {
    for (int tl = 0; tl < NUM_TIME_LEVELS; ++tl) {
      for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
        ...
        for (int igp = 0; igp < NP; ++igp) {
          for (int jgp = 0; jgp < NP; ++jgp) {
            dest_mirror(ie, tl, 0, igp, jgp, ilev)[ivec] =
                source(ie, tl, level, 0, igp, jgp);
            dest_mirror(ie, tl, 1, igp, jgp, ilev)[ivec] =
                source(ie, tl, level, 1, igp, jgp);
          }
        }
      }
    }
...
template <typename Source_T, typename Dest_T>
typename std::enable_if<
    (host_view_mappable<Source_T,
                        Real * [NUM_PHYSICAL_LEV][2][NP][NP]>::value &&
...
sync_to_device(Source_T source, Dest_T dest) {
  ...
  for (int ie = 0; ie < source.extent_int(0); ++ie) {
    for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
      ...
      for (int igp = 0; igp < NP; ++igp) {
        for (int jgp = 0; jgp < NP; ++jgp) {
          dest_mirror(ie, 0, igp, jgp, ilev)[ivec] =
              source(ie, level, 0, igp, jgp);
          dest_mirror(ie, 1, igp, jgp, ilev)[ivec] =
...
template <typename Source_T, typename Dest_T>
typename std::enable_if<(host_view_mappable<Source_T, Real * [NP][NP]>::value &&
...
typename std::enable_if<
    (host_view_mappable<Source_T,
                        Real * [Q_NUM_TIME_LEVELS][QSIZE_D][NUM_PHYSICAL_LEV]
                                                  [NP][NP]>::value &&
     exec_view_mappable<Dest_T, Scalar * [Q_NUM_TIME_LEVELS][QSIZE_D][NP][NP]
                                                            [NUM_LEV]>::value),
    void>::type
sync_to_device(Source_T source, Dest_T dest) {
  typename Dest_T::HostMirror dest_mirror = Kokkos::create_mirror_view(dest);
    ...
    for (int q_tl = 0; q_tl < Q_NUM_TIME_LEVELS; ++q_tl) {
      for (int q = 0; q < QSIZE_D; ++q) {
        for (int level = 0; level < NUM_PHYSICAL_LEV; ++level) {
          ...
          for (int igp = 0; igp < NP; ++igp) {
            for (int jgp = 0; jgp < NP; ++jgp) {
              dest_mirror(ie, q_tl, q, igp, jgp, ilev)[ivec] =
                  source(ie, q_tl, q, level, igp, jgp);
            }
          }
        }
      }
...
// These last two sync_to_host deal with views with NUM_INTERFACE_LEV levels on
// host, and either copy the whole view from a view with NUM_LEV_P level packs
// (1st version) , or copy only the first NUM_PHYSICAL_LEV from a view with
// NUM_LEV level packs (2nd version)
template <typename Source_T, typename Dest_T>
typename std::enable_if<

...
void prim_run_subcycle_c(const Real &dt, int &nstep, int &nm1, int &n0,
                         int &np1, const int &last_time_step) {
...
// Apply forcing
  ...
  Errors::runtime_abort(
      "CAM forcing not yet availble in C++.\n" Errors::err_not_implemented);
...
void update_q(const int np1_qdp, const int np1) {
  ...
  Kokkos::parallel_for(
      Kokkos::RangePolicy<ExecSpace>(0, num_elems * qsize * NP * NP * NUM_LEV),
      KOKKOS_LAMBDA(const int idx) {
        ...
        const Scalar dp = hyai_delta(level) * ps0 +
                          hybi_delta(level) * ps_v(ie, np1, igp, jgp);
  ...
  Kokkos::parallel_for(
      Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(
          0, num_elems * qsize * NP * NP * NUM_PHYSICAL_LEV),
      KOKKOS_LAMBDA(const int idx) {
